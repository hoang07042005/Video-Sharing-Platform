...
export default function CommunityPostCard({
      ...
      if (firstMedia.type === "image") {
        ...
        const checkOrientation = () => {
          ...
          setPrimaryOrientation(
            img.height >= img.width * 0.85 ? "portrait" : "landscape",
          );
        };
        ...
        vid.onloadedmetadata = () => {
          setPrimaryOrientation(
            vid.videoHeight >= vid.videoWidth * 0.85 ? "portrait" : "landscape",
          );
        };
  ...
  const handleVote = async (optionId) => {
    ...
    updatedOptions.forEach((opt) => {
      opt.votePercentage =
        newTotalVotes > 0
          ? Math.round((opt.votesCount / newTotalVotes) * 100)
          : 0;
    });
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `/api/community/poll/${optionId}/vote`,
        {},
        {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        },
  ...
  const handlePin = async () => {
    try {
      const response = await axios.put(
        `/api/channels/${post.channelId}/community/${post.id}/pin`,
        {},
        {
          headers: {
      ...
      toast.success(
        response.data.message ||
          (post.isPinned ? "Đã bỏ ghim bài viết!" : "Đã ghim bài viết!"),
      );
  ...
  return (
    <>
      <div className="bg-[#1a1a1a] rounded-xl p-4 border border-white/10 mb-4 hover:border-white/20 transition-colors">
          ...
          <div className="flex gap-3 items-center">
            ...
            <div>
              {post.authorName ? (
                ...
                <Link
                  to={`/c/${post.authorHandle || post.channelHandle}`}
              ...
              <div className="flex items-center gap-2 text-xs text-gray-400 mt-0.5">
                <span className="text-gray-500 text-xs">
                  {moment.utc(post.createdAt).local().locale("vi").fromNow()}
                </span>
                {post.isPinned && (
              ...
              {showOptions && (
                <div className="absolute right-0 mt-1 w-36 bg-[#2a2a2a] rounded-lg shadow-xl border border-white/10 py-1 z-10">
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-4 py-2 text-red-400 hover:bg-white/5 text-sm transition-colors"
                  >
        ...
        <p className="text-gray-200 mb-3 whitespace-pre-line text-sm leading-relaxed">
          {post.content}
        </p>
              ...
              >
                {mediaItems[0].type === "image" ? (
                  <img
                    ...
                    alt=""
                    className="max-w-full max-h-[450px] object-contain group-hover:brightness-90 transition-all"
                  />
                ) : (
              ...
              <div className="grid grid-cols-2 gap-1 bg-gray-900">
                {mediaItems.map((media, idx) => (
                  <div
                    ...
                    className="relative h-64 md:h-72 cursor-pointer group"
                  >
                    {media.type === "image" ? (
                      <img
                        src={media.url}
              ...
              <div className="grid grid-cols-2 gap-1 bg-gray-900 rounded-lg overflow-hidden border border-white/10">
                {mediaItems.map((media, idx) => (
                  <div
                    key={idx}
                    onClick={() => openLightbox(idx)}
                    className="relative h-36 md:h-44 cursor-pointer group"
                  >
                    {media.type === "image" ? (
                      <img
                        src={media.url}
        ...
        <div className="flex items-center gap-6 text-gray-400 mt-2 pt-2 border-t border-white/10">
          ...
          >
            <ThumbsUp
              className={`w-5 h-5 ${isLiked ? "fill-purple-500 text-purple-500" : "group-hover:text-purple-400"}`}
            />
