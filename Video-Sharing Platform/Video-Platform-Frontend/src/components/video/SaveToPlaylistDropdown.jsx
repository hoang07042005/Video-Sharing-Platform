...
export default function SaveToPlaylistDropdown({ videoId, onClose }) {
  ...
  const handleToggle = async (playlist) => {
    ...
    try {
      await axios.post(
        `/api/playlists/${playlist.id}/toggle-video`,
        { videoId },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
        ...
        {loading ? (
          ...
          playlists.map((playlist) => (
            ...
            >
              {/* Checkbox */}
              <div
                className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${playlist.containsVideo ? "bg-[#FF4E00] border-[#FF4E00]" : "border-gray-500"}`}
              >
                {playlist.containsVideo && (
                  <Check className="w-3 h-3 text-white" />
                )}
                {toggling === playlist.id && (
      ...
      {hasSaved && (
        <div className="border-t border-white/10 px-4 py-2">
          <p className="text-xs text-gray-400 text-center">
            Đã lưu vào {playlists.filter((p) => p.containsVideo).length} danh
            sách phát
          </p>
        </div>
      )}
