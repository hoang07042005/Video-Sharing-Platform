    ...
    try {
      if (modalMode === "add") {
        ...
        const res = await axios.put(
          `/api/admincategories/${currentCategory.id}`,
          currentCategory,
        );
        setCategories(
          categories.map((c) => (c.id === currentCategory.id ? res.data : c)),
        );
      }
      closeModal();
      fetchCategories();
    ...
    <div className="p-2 md:p-2 max-w-[1600px] mx-auto min-h-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Tạo, chỉnh sửa và quản lý danh mục video dễ dàng.
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6 justify-between">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#141418] hover:bg-[#0F0F0F] border border-white/10 text-gray-300 text-sm font-medium rounded-xl transition-colors shrink-0"
          >
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-white/5 bg-[#141418]/50">
                  ...
                  <th className="px-4 py-4 font-semibold text-gray-400 text-xs uppercase tracking-wider w-12">
                    #
                  </th>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-4">
          {loading ? (
            ...
            currentItems.map((c) => (
              <div
                key={c.id}
                className="bg-[#141418] rounded-xl p-5 border border-white/5 flex flex-col hover:border-purple-500/50 transition-colors group relative"
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {c.isActive ? (
                      ...
                      <span className="bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded text-[10px] font-medium">
                        Ẩn
                      </span>
                    )}
                    <div className="relative group/menu z-10">
                      ...
                      <div className="absolute right-0 top-full mt-1 w-36 bg-[#0F0F0F] border border-white/10 rounded-lg shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all py-1">
                        <button
                          onClick={() => openEditModal(c)}
                          className="w-full text-left px-4 py-2 text-xs font-medium text-gray-300 hover:bg-white/5 flex items-center gap-2 cursor-pointer"
                        >
      ...
      <div className="p-4 border-t border-white/5 flex items-center justify-between text-sm">
        <span className="text-gray-500">
          Hiển thị {filteredCategories.length === 0 ? 0 : startIndex + 1} -{" "}
          {Math.min(startIndex + itemsPerPage, filteredCategories.length)} trong
            ...
            <div className="p-6 space-y-5 overflow-y-auto sidebar-scroll">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    />
                    <div
                      className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${currentCategory.isActive ? "border-emerald-500" : "border-gray-500 group-hover:border-gray-400"}`}
                    >
                    ...
                    <span
                      className={`text-sm font-medium ${currentCategory.isActive ? "text-emerald-400" : "text-gray-400"}`}
                    >
                      Hiển thị
                    </span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    />
                    <div
                      className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${!currentCategory.isActive ? "border-red-500" : "border-gray-500 group-hover:border-gray-400"}`}
                    >
                      {!currentCategory.isActive && (
                    ...
                    <span
                      className={`text-sm font-medium ${!currentCategory.isActive ? "text-red-400" : "text-gray-400"}`}
                    >
                      Ẩn
                    </span>
                  </label>
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-500 mt-2 flex items-center gap-1">
                  <LucideIcons.Info className="w-3.5 h-3.5" /> Biểu tượng sẽ
                  được hiển thị trên Sidebar và Trang chủ.
                </p>
              </div>
            </div>
