...
export default function AdminViolations() {
  ...
  const fetchViolations = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get("/api/admin/violations?page=1&pageSize=50", {
        headers,
      });
  ...
  const fetchLiveMessageContent = async (messageId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const headers = { Authorization: `Bearer ${token}` };
      const res = await axios.get(`/api/livemessages/${messageId}`, {
        headers,
      });
  ...
  const handleViolationAction = async (violation, action) => {
    try {
      ...
      await axios.post(
        "/api/admin/reports/handle-violation",
        {
          targetId,
          targetType,
          action,
      ...
      setViolations((prev) =>
        prev.filter(
          (item) =>
            ...
            (violation.originalId || violation.id),
        ),
      );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Quản lý các nội dung và tài khoản vi phạm chính sách cộng đồng
          </p>
        </div>
        ...
        <div className="bg-[#141418] border border-white/5 p-4 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-white mt-1">
              {violations.filter((v) => v.targetType === "Video").length}
            </p>
          </div>
        </div>
        <div className="bg-[#141418] border border-white/5 p-4 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-white mt-1">
              {violations.filter((v) => v.targetType === "Comment").length}
            </p>
          </div>
        </div>
        <div className="bg-[#141418] border border-white/5 p-4 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-white mt-1">
              {violations.filter((v) => v.targetType === "User").length}
            </p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="py-12 text-center">
                    ...
                    <p className="text-gray-400 text-sm mt-1">
                      Tuyệt vời! Cộng đồng đang hoạt động rất tốt.
                    </p>
                  </td>
                </tr>
              ) : (
                violations.map((violation, idx) => (
                  ...
                  >
                    <td className="py-4 px-4 text-xs text-red-400 font-mono font-bold">
                      {violation.id}
                    </td>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-white">
                            {violation.targetType || "Không rõ"}
                          </span>
                          <span className="text-[10px] text-gray-500 font-mono">
                            ID: {violation.targetId?.substring(0, 8) || "N/A"}
                            ...
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="py-4 px-4">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${violation.pColor}`}
                      >
      ...
      {isDetailOpen && selectedViolation && (
        ...
        >
          <div
            className="w-full h-full max-w-2xl border border-white/10 bg-[#141418] p-6 shadow-2xl"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-2 flex items-center gap-3">
                  {selectedViolation.avatar?.startsWith("http") ? (
                    <img
                      src={selectedViolation.avatar}
                      alt={selectedViolation.user}
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 font-semibold text-white">
                  {selectedViolation.targetType || "Không rõ"}
                </p>
              </div>
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <span
                  className={`mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${selectedViolation.pColor}`}
                >
                  <ShieldAlert className="w-3 h-3" />
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                {loadingMessage ? (
                  ...
                  <div className="mt-2 bg-[#0F0F0F] rounded-lg p-3 border border-white/5">
                    <p className="text-sm leading-6 text-white break-words">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    Không thể tải nội dung bình luận
                  </p>
                )}
              </div>
            )}
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-3">
              {selectedViolation.targetType === "Comment" && (
                <>
                  <button
                    onClick={() =>
                      handleViolationAction(selectedViolation, "hide")
                    }
                    disabled={
                      actionLoadingId ===
                      (selectedViolation.originalId || selectedViolation.id)
                    }
                  ...
                  <button
                    onClick={() =>
                      handleViolationAction(selectedViolation, "delete")
                    }
                    disabled={
                      actionLoadingId ===
                      (selectedViolation.originalId || selectedViolation.id)
              ...
              {(selectedViolation.targetType === "LiveMessage" ||
                selectedViolation.targetType === "User") && (
                <button
                  onClick={() =>
                    handleViolationAction(
                      selectedViolation,
                      selectedViolation.targetType === "User" ? "ban" : "hide",
                    )
                  }
                  disabled={
                    actionLoadingId ===
              ...
              <button
                onClick={() =>
                  handleViolationAction(selectedViolation, "ignore")
                }
                disabled={
                  actionLoadingId ===
                  (selectedViolation.originalId || selectedViolation.id)
