...
import { 
  Calendar, Download, AlertTriangle, Flag, ShieldAlert, 
  ShieldCheck, CheckCircle2, Eye, MoreVertical,
  ChevronLeft, ChevronRight, Activity, ArrowUpRight, ArrowDownRight, Loader2
} from 'lucide-react';
import { 
...
export default function AdminReports() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const statsRes = await axios.get('/api/admin/reports/stats', { headers });
        setStats(statsRes.data);

        const reportsRes = await axios.get(`/api/admin/reports?page=${page}&pageSize=5`, { headers });
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.put(`/api/admin/reports/${id.replace('#BC-', '')}/status`, { status: newStatus });
      // Refresh data
      setPage(1);
      const statsRes = await axios.get('/api/admin/reports/stats');
      setStats(statsRes.data);
      const reportsRes = await axios.get(`/api/admin/reports?page=1&pageSize=5`);
      setReports(reportsRes.data);
    } catch (error) {
  ...
  const priorityData = [
    { name: 'Nghiêm trọng', value: stats.severePriority || 0, color: '#EF4444' },
    { name: 'Cảnh báo', value: stats.warningPriority || 0, color: '#F97316' },
    { name: 'Kiểm tra lại', value: stats.reviewPriority || 0, color: '#EAB308' },
  ];
  
  const totalPriority = stats.severePriority + stats.warningPriority + stats.reviewPriority || 1;
  ...
  const handleQuickAction = async (report, newStatus) => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.put(`/api/admin/reports/${report.originalId || report.id.replace('#BC-', '')}/status`, { status: newStatus }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      closeReportDetail();
      const [statsRes, reportsRes] = await Promise.all([
        axios.get('/api/admin/reports/stats', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`/api/admin/reports?page=${page}&pageSize=5`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
      setStats(statsRes.data);
      setReports(reportsRes.data);
    } catch (error) {
    ...
    <div className="pb-20 max-w-[1600px] mx-auto text-white">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm">Tổng quan tình hình báo cáo, khiếu nại và vi phạm trong hệ thống</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 mb-6">
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 overflow-hidden flex flex-col justify-between pt-5">
          <div className="px-5 flex gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-300">Tổng báo cáo mới</p>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-3xl font-bold text-white">{stats.totalReports}</span>
                <span className={`text-xs font-semibold flex items-center ${stats.totalReportsTrend >= 0 ? 'text-red-500' : 'text-emerald-500'}`}>
                  {stats.totalReportsTrend >= 0 ? <ArrowUpRight className="w-3 h-3 mr-0.5"/> : <ArrowDownRight className="w-3 h-3 mr-0.5"/>} 
                  {Math.abs(stats.totalReportsTrend)}%
                </span>
              </div>
              <p className="text-[10px] text-gray-500 mt-0.5">So với 7 ngày trước</p>
            </div>
          </div>
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 overflow-hidden flex flex-col justify-between pt-5">
          <div className="px-5 flex gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-300">Khiếu nại chưa giải quyết</p>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-3xl font-bold text-white">{stats.pendingReports}</span>
                <span className={`text-xs font-semibold flex items-center ${stats.pendingReportsTrend >= 0 ? 'text-orange-500' : 'text-emerald-500'}`}>
                  {stats.pendingReportsTrend >= 0 ? <ArrowUpRight className="w-3 h-3 mr-0.5"/> : <ArrowDownRight className="w-3 h-3 mr-0.5"/>}
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 overflow-hidden flex flex-col justify-between pt-5">
          <div className="px-5 flex gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-300">Vi phạm bản quyền</p>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-3xl font-bold text-white">{stats.copyrightReports}</span>
                <span className={`text-xs font-semibold flex items-center ${stats.copyrightReportsTrend >= 0 ? 'text-purple-500' : 'text-emerald-500'}`}>
                  {stats.copyrightReportsTrend >= 0 ? <ArrowUpRight className="w-3 h-3 mr-0.5"/> : <ArrowDownRight className="w-3 h-3 mr-0.5"/>}
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 overflow-hidden flex flex-col justify-between pt-5">
          <div className="px-5 flex gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-300">Đã xử lý (Tuần này)</p>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-3xl font-bold text-white">{stats.resolvedThisWeek}</span>
                <span className={`text-xs font-semibold flex items-center ${stats.resolvedThisWeekTrend >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                  {stats.resolvedThisWeekTrend >= 0 ? <ArrowUpRight className="w-3 h-3 mr-0.5"/> : <ArrowDownRight className="w-3 h-3 mr-0.5"/>}
                  {Math.abs(stats.resolvedThisWeekTrend)}%
                </span>
              </div>
              <p className="text-[10px] text-gray-500 mt-0.5">So với tuần trước</p>
            </div>
          </div>
          <div className="h-10 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-[#141418] p-5 rounded-2xl border border-white/5 lg:col-span-2 h-[420px] flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-bold text-white">Xu hướng báo cáo</h3>
            </div>
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={reportsTimelineData} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorSpam" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#F97316" stopOpacity={0.3}/>
                    <stop offset="95%" stopColor="#F97316" stopOpacity={0}/>
                  </linearGradient>
                  <linearGradient id="colorInappropriate" x1="0" y1="0" x2="0" y2="1">
                ...
                <RechartsTooltip 
                  contentStyle={{ backgroundColor: '#15171f', borderColor: '#374151', color: '#fff', borderRadius: '8px', fontSize: '12px' }}
                />
                <Area type="monotone" dataKey="inappropriate" name="Nội dung xấu" stroke="#EF4444" strokeWidth={3} fillOpacity={1} fill="url(#colorInappropriate)" activeDot={{ r: 6, fill: '#15171f', stroke: '#EF4444', strokeWidth: 3 }} />
                <Area type="monotone" dataKey="spam" name="Spam" stroke="#F97316" strokeWidth={3} fillOpacity={1} fill="url(#colorSpam)" activeDot={{ r: 6, fill: '#15171f', stroke: '#F97316', strokeWidth: 3 }} />
                <Area type="monotone" dataKey="copyright" name="Bản quyền" stroke="#A855F7" strokeWidth={3} fillOpacity={1} fill="url(#colorCopyright)" activeDot={{ r: 6, fill: '#15171f', stroke: '#A855F7', strokeWidth: 3 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-8">
            ...
            <div className="relative w-[180px] h-[180px]">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-2xl font-bold text-white leading-none my-1">{totalPriority}</span>
            ...
            <div className="flex flex-col gap-4 w-full px-4">
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <div className="w-2.5 h-2.5 rounded-full bg-red-500"></div>
                  <span className="text-xs text-gray-300">Nghiêm trọng (Khóa ngay)</span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-white font-semibold">{stats.severePriority}</span>
                  <span className="text-gray-500">({Math.round((stats.severePriority / totalPriority) * 100)}%)</span>
                </div>
              </div>
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <div className="w-2.5 h-2.5 rounded-full bg-orange-500"></div>
                  <span className="text-xs text-gray-300">Cảnh báo (Warning)</span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-white font-semibold">{stats.warningPriority}</span>
                  <span className="text-gray-500">({Math.round((stats.warningPriority / totalPriority) * 100)}%)</span>
                </div>
              </div>
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <div className="w-2.5 h-2.5 rounded-full bg-yellow-500"></div>
                  <span className="text-xs text-gray-300">Kiểm tra lại (Review)</span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-white font-semibold">{stats.reviewPriority}</span>
                  <span className="text-gray-500">({Math.round((stats.reviewPriority / totalPriority) * 100)}%)</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-[#141418] p-5 rounded-2xl border border-white/5">
                ...
                <tr key={idx} className="group hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex flex-col">
                      ...
                      {report.description && (
                        <span className="text-[10px] text-gray-500 mt-1 line-clamp-1" title={report.description}>
                          {report.description}
                        </span>
                      )}
                    </div>
                  </td>
                  <td className="py-4 px-4 text-xs text-gray-400">{report.time}</td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <span className={`text-xs font-semibold flex items-center gap-1.5 ${report.status === 'Chờ xử lý' ? 'text-orange-500' : 'text-emerald-500'}`}>
                  ...
                  <td className="py-4 px-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <div className="relative group/menu">
                        <button className="p-1.5 text-gray-500 hover:text-white rounded-lg transition-colors cursor-pointer" title="Khác">
                        ...
                        <div className="absolute right-0 top-full mt-1 w-36 bg-[#1a1c23] border border-white/5 rounded-lg shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-20 overflow-hidden">
                          {report.status !== 'Đã giải quyết' && (
                            <button onClick={() => handleStatusChange(report.id, 'Resolved')} className="w-full text-left px-4 py-2 text-xs text-emerald-400 hover:bg-white/5 transition-colors">
                              Đánh dấu đã giải quyết
                            </button>
                          )}
                          <button onClick={() => handleStatusChange(report.id, 'Ignored')} className="w-full text-left px-4 py-2 text-xs text-gray-400 hover:bg-white/5 transition-colors">
                            Bỏ qua
                          </button>
                        </div>
                      </div>
                ...
                {selectedReport.status !== 'Đã giải quyết' && (
                  <>
                    <button
                      ...
                      disabled={actionLoadingId === (selectedReport.originalId || selectedReport.id)}
                    ...
                    >
                      {actionLoadingId === (selectedReport.originalId || selectedReport.id) ? 'Đang xử lý...' : 'Đánh dấu đã giải quyết'}
                    </button>
                    <button
                      onClick={() => handleQuickAction(selectedReport, 'Ignored')}
                      disabled={actionLoadingId === (selectedReport.originalId || selectedReport.id)}
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/5 relative">
          ...
          <div className="text-xs text-gray-500 text-right absolute right-4 hidden md:block">
            Hiển thị {recentReports.length > 0 ? (page - 1) * 5 + 1 : 0} - {Math.min(page * 5, stats.totalReports)} trong {stats.totalReports} báo cáo
          </div>
        </div>
      </div>
