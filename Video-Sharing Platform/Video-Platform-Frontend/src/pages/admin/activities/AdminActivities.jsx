...
import { 
  Activity, Search, Calendar, Filter, Download, 
  ChevronRight, RefreshCw, Loader2,
  CheckCircle2, AlertTriangle, XCircle, LayoutGrid,
  Video, Tv, User, Settings, AlertCircle, DollarSign, Ban, ChevronLeft, ChevronRight as IconChevronRight
} from 'lucide-react';
...
const SparklineGreen = () => (
  <svg width="60" height="20" viewBox="0 0 60 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M1 15C5.5 15 8.5 12 13 8C17.5 4 20.5 4 25 8C29.5 12 32.5 16 37 12C41.5 8 44.5 4 49 8C53.5 12 56.5 6 59 2" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const SparklineYellow = () => (
  <svg width="60" height="20" viewBox="0 0 60 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M1 15C5.5 15 8.5 16 13 14C17.5 12 20.5 14 25 12C29.5 10 32.5 12 37 8C41.5 4 44.5 6 49 4C53.5 2 56.5 6 59 4" stroke="#eab308" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const SparklineRed = () => (
  <svg width="60" height="20" viewBox="0 0 60 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M1 4C5.5 4 8.5 6 13 8C17.5 10 20.5 6 25 8C29.5 10 32.5 12 37 10C41.5 8 44.5 14 49 14C53.5 14 56.5 16 59 18" stroke="#ef4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const SparklinePurple = () => (
  <svg width="60" height="20" viewBox="0 0 60 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M1 18C5.5 18 8.5 14 13 14C17.5 14 20.5 10 25 8C29.5 6 32.5 10 37 6C41.5 2 44.5 6 49 4C53.5 2 56.5 4 59 2" stroke="#a855f7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    <defs>
      <linearGradient id="paint0_linear" x1="30" y1="2" x2="30" y2="18" gradientUnits="userSpaceOnUse">
...
const AdminActivities = () => {
    ...
    try {
      ...
      const enhancedData = response.data.map((item, index) => {
        ...
        return {
          ...
          browser: index % 3 === 0 ? 'Firefox • Windows' : index % 5 === 0 ? 'Safari • macOS' : 'Chrome • Windows',
        };
      });
  ...
  const getCategoryInfo = (target) => {
    ...
    if (targetStr.startsWith('video')) {
      return { id: 'video', name: 'Video', icon: <Video className="w-5 h-5 text-blue-400" />, bg: 'bg-blue-500/10' };
    }
    if (targetStr.startsWith('channel')) {
      return { id: 'channel', name: 'Kênh', icon: <Tv className="w-5 h-5 text-green-400" />, bg: 'bg-green-500/10' };
    }
    if (targetStr.startsWith('user')) {
      return { id: 'user', name: 'Người dùng', icon: <User className="w-5 h-5 text-purple-400" />, bg: 'bg-purple-500/10' };
    }
    if (targetStr.startsWith('role')) {
      return { id: 'role', name: 'Phân quyền', icon: <Settings className="w-5 h-5 text-yellow-400" />, bg: 'bg-yellow-500/10' };
    }
    if (targetStr.startsWith('systemsettings')) {
      return { id: 'system', name: 'Cài đặt', icon: <Settings className="w-5 h-5 text-gray-400" />, bg: 'bg-gray-400/10' };
    }
    if (targetStr.startsWith('report')) {
      return { id: 'report', name: 'Báo cáo', icon: <AlertCircle className="w-5 h-5 text-red-400" />, bg: 'bg-red-500/10' };
    }
    
    return { id: 'other', name: 'Khác', icon: <LayoutGrid className="w-5 h-5 text-blue-400" />, bg: 'bg-blue-500/10' };
  };
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    if (searchTerm) {
      ...
      const matchesSearch = 
        ...
        (activity.target && activity.target.toLowerCase().includes(searchLower)) ||
    ...
    if (timeFilter !== 'all') {
      ...
      const thresholdDate = new Date(now.getTime() - (diffDays * 24 * 60 * 60 * 1000));
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'Thành công':
        return <span className="px-3 py-1 rounded-md text-[13px] font-medium text-green-400 border border-green-500/20 bg-green-500/10">Thành công</span>;
      case 'Cảnh báo':
        return <span className="px-3 py-1 rounded-md text-[13px] font-medium text-yellow-400 border border-yellow-500/20 bg-yellow-500/10">Cảnh báo</span>;
      case 'Thất bại':
        return <span className="px-3 py-1 rounded-md text-[13px] font-medium text-red-400 border border-red-500/20 bg-red-500/10">Thất bại</span>;
      default:
        return <span className="px-3 py-1 rounded-md text-[13px] font-medium text-gray-400 border border-gray-500/20 bg-gray-500/10">{status}</span>;
    }
  };

  const getStatusDotColor = (status) => {
    switch(status) {
      case 'Thành công': return 'bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.8)]'; // Using purple dot like the image for success? Actually image uses purple/blue for some
      case 'Cảnh báo': return 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.8)]';
      case 'Thất bại': return 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]';
    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-8 min-h-screen font-sans text-gray-200">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-2 text-sm">Theo dõi và quản lý tất cả lịch sử hoạt động và sự kiện trong hệ thống.</p>
        </div>
        ...
        <div className="bg-[#141418] border border-white/5 rounded-xl p-5 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-sm font-medium text-gray-400">Tổng hoạt động</p>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                <th className="py-4 px-6 font-semibold min-w-[120px]">THỜI GIAN</th>
                <th className="py-4 px-6 font-semibold min-w-[280px]">HOẠT ĐỘNG</th>
                <th className="py-4 px-6 font-semibold min-w-[200px]">NGƯỜI THỰC HIỆN</th>
                <th className="py-4 px-6 font-semibold min-w-[180px]">ĐỊA CHỈ IP</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan="5" className="py-16 text-center">
                    ...
                    <p className="text-gray-400">Đang tải lịch sử hoạt động...</p>
                  </td>
                </tr>
                ...
                currentItems.map((activity, idx) => {
                  ...
                  const [timePart, datePart] = activity.time ? activity.time.split(' ') : ['--:--', '--/--/----'];
                  
                  return (
                    <tr key={activity.id || idx} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                        ...
                        <div className="flex items-start gap-3">
                          <div className="mt-1.5 relative flex items-center justify-center">
                            <div className="w-5 h-5 rounded-full bg-[#0F0F0F] flex items-center justify-center border border-white/5 z-10">
                               <div className={`w-2 h-2 rounded-full ${getStatusDotColor(activity.status)}`}></div>
                            </div>
                          ...
                          <div>
                            <div className="text-[13px] text-gray-300 font-medium font-mono">{timePart}</div>
                            <div className="text-[11px] text-gray-500 mt-0.5">{datePart}</div>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="text-[14px] text-gray-200 font-semibold mb-0.5">{activity.action}</div>
                            <div className="text-[12px] text-gray-500 line-clamp-1">{activity.details}</div>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-[13px] text-gray-300">{activity.user}</span>
                      </td>
                      <td className="py-4 px-6">
                        <div className="text-[13px] text-gray-300 font-mono mb-0.5">{activity.ipAddress}</div>
                        <div className="text-[11px] text-gray-500">{activity.browser}</div>
                      </td>
                      <td className="py-4 px-6">
        ...
        {!loading && filteredActivities.length > 0 && (
          <div className="px-6 py-4 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-[13px] text-gray-500">
              Hiển thị <span className="font-semibold text-white">{indexOfFirstItem + 1} - {Math.min(indexOfLastItem, totalActivities)}</span> trong tổng số <span className="font-semibold text-white">{totalActivities}</span> hoạt động
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-1">
                ...
                {currentPage < totalPages && (
                  <button onClick={() => setCurrentPage(currentPage + 1)} className="w-8 h-8 flex items-center justify-center rounded-md border border-white/5 bg-[#0F0F0F] text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-xs">
                    {currentPage + 1}
                  </button>
                )}
                
                {currentPage < totalPages - 1 && (
                  <button onClick={() => setCurrentPage(currentPage + 2)} className="w-8 h-8 flex items-center justify-center rounded-md border border-white/5 bg-[#0F0F0F] text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-xs">
                    {currentPage + 2}
                  </button>
                )}
                ...
                {totalPages > 3 && currentPage < totalPages - 2 && (
                  <button onClick={() => setCurrentPage(totalPages)} className="w-8 h-8 flex items-center justify-center rounded-md border border-white/5 bg-[#0F0F0F] text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-xs">
                    {totalPages}
                  </button>
                )}
