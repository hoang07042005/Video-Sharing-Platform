...
export default function AdminDashboard() {
  ...
  const PIE_COLORS = ['#3B82F6', '#8B5CF6', '#EC4899', '#F59E0B', '#10B981', '#EF4444', '#02ddffff', '#F97316', '#6366F1', '#F43F5E'];
  ...
  const renderMiniChart = (values, color, type = 'area') => {
    ...
    if (type === 'area') {
      ...
      return (
        <svg width="100%" height="100%" viewBox="-2 0 104 100" preserveAspectRatio="none" className="opacity-80">
          <defs>
          ...
          <path d={`M ${points.map(p => p.replace(',',' ')).join(' L ')}`} fill="none" stroke={color} strokeWidth="2" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" />
          {points.map((p, i) => { const [x, y] = p.split(','); return <circle key={i} cx={x} cy={y} r="2" fill={color} vectorEffect="non-scaling-stroke" />; })}
        </svg>
      );
    } else if (type === 'bar') {
      ...
      return (
        <svg width="100%" height="100%" viewBox="-2 0 104 100" preserveAspectRatio="none" className="opacity-80">
          {points.map((p, i) => { 
            ...
            return <rect key={i} x={x - barWidth / 2} y={y} width={barWidth} height={100 - y} fill={color} opacity="0.8" rx="1" />; 
          })}
        </svg>
      );
    } else if (type === 'line') {
      return (
        <svg width="100%" height="100%" viewBox="-2 0 104 100" preserveAspectRatio="none" className="opacity-80">
          <path d={`M ${points.map(p => p.replace(',',' ')).join(' L ')}`} fill="none" stroke={color} strokeWidth="2" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" />
          {points.map((p, i) => { const [x, y] = p.split(','); return <circle key={i} cx={x} cy={y} r="2.5" fill={color} vectorEffect="non-scaling-stroke" />; })}
        </svg>
      );
    } else if (type === 'donut') {
      const percent = Math.min(100, Math.max(0, values[values.length-1] || 75));
      return (
        <div className="flex justify-between items-end h-full w-full px-4 pb-2">
          ...
          <div className="relative w-12 h-11 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
              ...
              <path className="text-blue-500" strokeDasharray={`${percent}, 100`} strokeWidth="4" stroke="currentColor" fill="none" strokeLinecap="round" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
            </svg>
            <span className="absolute text-[9px] font-bold text-white">{Math.round(percent)}%</span>
          </div>
        </div>
      );
    } else if (type === 'smooth') {
    ...
    <div className="pb-20 max-w-[1600px] mx-auto text-white px-1">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Theo dõi số liệu thống kê và hiệu suất hoạt động của hệ thống.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative" ref={datePickerRef}>
            <button onClick={() => setShowDatePicker(v => !v)} className="flex items-center gap-2 bg-[#1a1c23] border border-white/10 px-4 py-2 rounded-lg text-sm text-gray-300 w-fit hover:border-white/20 transition-colors">
              <Calendar className="w-4 h-4 text-gray-500" />
              <span>{activeMonth ? (() => { const [y, m] = activeMonth.split('-').map(Number); return `Tháng ${m}/${y}`; })() : 'Tất cả thời gian'}</span>
              <svg className={`w-3 h-3 ml-2 text-gray-500 transition-transform ${showDatePicker ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"/></svg>
            </button>
            {showDatePicker && (
              <div className="absolute right-0 top-full mt-2 z-50 bg-[#1a1c23] border border-white/10 rounded-xl shadow-2xl p-4 w-[260px]">
                <p className="text-xs font-semibold text-gray-400 mb-3">Chọn tháng</p>
                <input type="month" value={tempMonth} max={`${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}`} onChange={e => setTempMonth(e.target.value)} className="w-full bg-[#0f1117] border border-white/10 rounded-lg px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-purple-500/60 [color-scheme:dark]" />
                {tempMonth && <p className="text-[10px] text-gray-500 mt-2">{(() => { const [y,m] = tempMonth.split('-').map(Number); const ld = new Date(y,m,0).getDate(); return `01/${String(m).padStart(2,'0')}/${y} — ${String(ld).padStart(2,'0')}/${String(m).padStart(2,'0')}/${y}`; })()}</p>}
                <div className="flex gap-2 mt-4">
                  <button onClick={() => setShowDatePicker(false)} className="flex-1 py-2 rounded-lg text-xs text-gray-400 bg-white/5 hover:bg-white/10 transition-colors">Đóng</button>
                  <button onClick={() => { if (!tempMonth) { setActiveMonth(''); setShowDatePicker(false); setIsFiltered(false); setRefetching(true); fetchAllData({startDate:'2000-01-01',endDate:'2099-12-31',days:36500}); return; } const [y,m] = tempMonth.split('-').map(Number); const startStr=`${y}-${String(m).padStart(2,'0')}-01`; const ld=new Date(y,m,0).getDate(); const endStr=`${y}-${String(m).padStart(2,'0')}-${String(ld).padStart(2,'0')}`; setActiveMonth(tempMonth); setShowDatePicker(false); setIsFiltered(true); setRefetching(true); fetchAllData({startDate:startStr,endDate:endStr,days:ld}); }} className="flex-1 py-2 rounded-lg text-xs font-semibold text-white bg-purple-600 hover:bg-purple-500 transition-colors">Áp dụng</button>
                </div>
              </div>
            )}
          </div>
          <button onClick={() => { if(!isFiltered) return; const now=new Date(); const thisMonth=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`; setActiveMonth(''); setTempMonth(thisMonth); setIsFiltered(false); setRefetching(true); fetchAllData({startDate:'2000-01-01',endDate:'2099-12-31',days:36500}); }} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs border transition-all ${isFiltered ? 'text-red-400 bg-red-500/10 border-red-500/20 hover:bg-red-500/20 cursor-pointer' : 'text-gray-600 bg-white/5 border-white/5 cursor-not-allowed opacity-40'}`}>
      ...
      <div className="grid grid-cols-3 lg:grid-cols-6 gap-3 mb-3 h-[240px]">
        {[
          { icon: <Users className="w-5 h-5 text-purple-400"/>, bg:'bg-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.2)]', label:'Người dùng', value:stats?.totalUsers?.toLocaleString()??'0', growth:stats?.userGrowth, chart:renderMiniChart(buildTrendSeries(stats?.totalUsers??0,stats?.userGrowth??0),'#A78BFA','line') },
          { icon: <Video className="w-5 h-5 text-blue-400"/>, bg:'bg-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.2)]', label:'Tổng video', value:stats?.totalVideos?.toLocaleString()??'0', growth:stats?.videoGrowth, chart:renderMiniChart(buildTrendSeries(stats?.totalVideos??0,stats?.videoGrowth??0),'#3B82F6','bar') },
          { icon: <Eye className="w-5 h-5 text-orange-400"/>, bg:'bg-orange-500/20 shadow-[0_0_15px_rgba(245,158,11,0.2)]', label:'Lượt xem', value:stats?.totalViews?.toLocaleString()??'0', growth:stats?.viewsGrowth, chart:renderMiniChart(buildTrendSeries(stats?.totalViews??0,stats?.viewsGrowth??0),'#F59E0B','area') },
          { icon: <DollarSign className="w-5 h-5 text-green-400"/>, bg:'bg-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.2)]', label:'Doanh thu', value:`${(stats?.monthlyRevenue??0).toLocaleString('en-US')}₫`, growth:stats?.revenueGrowth, chart:renderMiniChart(buildTrendSeries(stats?.monthlyRevenue??0,stats?.revenueGrowth??0),'#22C55E','bar') },
          { icon: <Database className="w-5 h-5 text-blue-400"/>, bg:'bg-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.2)]', label:'Lưu trữ', value:`${stats?.totalStorageGB??0} GB`, growth:stats?.storageGrowth, chart:renderMiniChart(buildTrendSeries(stats?.totalStorageGB??0,stats?.storageGrowth??0),'#3B82F6','donut') },
          { icon: <Heart className="w-5 h-5 text-pink-400"/>, bg:'bg-pink-500/20 shadow-[0_0_15px_rgba(236,72,153,0.2)]', label:'Lượt thích', value:stats?.totalLikes?.toLocaleString()??'0', growth:stats?.likesGrowth, chart:renderMiniChart(buildTrendSeries(stats?.totalLikes??0,stats?.likesGrowth??0),'#EC4899','smooth') },
        ].map((card, i) => (
          <div key={i} className="bg-[#141418] rounded-xl border border-white/5 relative overflow-hidden flex flex-col h-[230px] pt-7 px-7 pb-0">
            <div className="flex flex-col z-10 relative">
              <div className="flex items-center gap-3 mb-3">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${card.bg}`}>
                  {card.icon}
                </div>
                <span className="text-gray-400 text-[14px] font-medium truncate">{card.label}</span>
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-white text-2xl font-bold leading-none mb-2 truncate">{card.value}</span>
                <div className="flex items-center justify-between gap-1.5">
                  <span className={`text-[12px] font-bold ${card.growth >= 0 ? 'text-green-400' : 'text-red-400'} whitespace-nowrap`}>
                    {card.growth >= 0 ? '↑' : '↓'} {Math.abs(card.growth ?? 0).toFixed(1)}%
                  </span>
                  <span className="text-gray-500 text-[10px] mt-1 truncate">so với 7 ngày trước</span>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_2fr_1.4fr] gap-3 mb-3">
        ...
        <div className="bg-[#141418] p-4 rounded-xl border border-white/5 flex flex-col h-[280px]">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xs font-semibold text-white">Xu hướng hiệu suất tổng quan</h3>
            <select value={chartDays} onChange={handleChartDaysChange} className="bg-[#1a1c23] border border-white/10 px-2 py-1 rounded text-[10px] text-gray-400 outline-none cursor-pointer">
              <option value="7">7 ngày qua</option>
              <option value="16">16 ngày qua</option>
              <option value="30">30 ngày qua</option>
              {chartDays === 'custom' && <option value="custom">Theo bộ lọc chung</option>}
            </select>
          </div>
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData?.traffic} margin={{ top:5, right:5, left:-28, bottom:0 }}>
                <defs>
                  <linearGradient id="gViews" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8B5CF6" stopOpacity={0.3}/><stop offset="95%" stopColor="#8B5CF6" stopOpacity={0}/></linearGradient>
                  <linearGradient id="gUsers" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#F59E0B" stopOpacity={0.3}/><stop offset="95%" stopColor="#F59E0B" stopOpacity={0}/></linearGradient>
                  <linearGradient id="gRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#34D399" stopOpacity={0.3}/><stop offset="95%" stopColor="#34D399" stopOpacity={0}/></linearGradient>
                </defs>
                <XAxis dataKey="name" stroke="#4b5563" tick={{fontSize:9}} axisLine={false} tickLine={false}/>
                <YAxis stroke="#4b5563" tick={{fontSize:9}} axisLine={false} tickLine={false}/>
                <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" vertical={false}/>
                <Tooltip contentStyle={{ backgroundColor:'#1a1c23', borderColor:'#374151', color:'#fff', borderRadius:'8px', fontSize:'11px' }}/>
                <Area type="monotone" dataKey="views" stroke="#8B5CF6" strokeWidth={2} fillOpacity={1} fill="url(#gViews)" name="Lượt xem" dot={false}/>
                <Area type="monotone" dataKey="revenue" stroke="#34D399" strokeWidth={2} fillOpacity={1} fill="url(#gRevenue)" name="Doanh thu" dot={false}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-[#141418] p-4 rounded-xl border border-white/5 flex flex-col h-[280px]">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xs font-semibold text-white">Nâng cấp tài khoản vs Đăng kí hội viên</h3>
            <select value={chartDays} onChange={handleChartDaysChange} className="bg-[#1a1c23] border border-white/10 px-2 py-1 rounded text-[10px] text-gray-400 outline-none cursor-pointer">
              <option value="7">7 ngày qua</option>
              <option value="16">16 ngày qua</option>
              <option value="30">30 ngày qua</option>
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData?.accountUpgradesVsRegistrations} margin={{ top:5, right:5, left:-28, bottom:0 }} barCategoryGap="30%" barGap={3}>
                <defs>
                  <linearGradient id="bUpg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3B82F6"/><stop offset="100%" stopColor="#1D4ED8" stopOpacity={0.8}/></linearGradient>
                  <linearGradient id="bReg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#10B981"/><stop offset="100%" stopColor="#047857" stopOpacity={0.8}/></linearGradient>
                </defs>
                <XAxis dataKey="name" stroke="#4b5563" tick={{fontSize:9}} axisLine={false} tickLine={false}/>
                <YAxis stroke="#4b5563" tick={{fontSize:9}} axisLine={false} tickLine={false}/>
                <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" vertical={false}/>
                <Tooltip contentStyle={{ backgroundColor:'#1a1c23', borderColor:'#374151', color:'#fff', borderRadius:'8px', fontSize:'11px' }} cursor={{fill:'rgba(255,255,255,0.04)'}}/>
                <Bar dataKey="upgrades" fill="url(#bUpg)" radius={[3,3,0,0]} name="Nâng cấp tài khoản" maxBarSize={22}/>
                <Bar dataKey="registrations" fill="url(#bReg)" radius={[3,3,0,0]} name="Đăng kí hội viên" maxBarSize={22}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-[#141418] p-4 rounded-xl border border-white/5 h-[280px] flex flex-col">
          <h3 className="text-xs font-semibold text-white mb-3">Tình hình hệ thống</h3>
          <div className="grid grid-cols-2 gap-2 flex-1">
            {[
              { icon: <AlertTriangle className="w-3.5 h-3.5 text-red-400"/>, bg:'bg-red-500/20', label:'Báo cáo vi phạm', value:stats?.recentReports?.length??0, trend:'↑ 15%', trendColor:'text-green-400' },
              { icon: <Flag className="w-3.5 h-3.5 text-orange-400"/>, bg:'bg-orange-500/20', label:'Khiếu nại mới', value:17, trend:'↑ 8%', trendColor:'text-green-400' },
              { icon: <VideoIcon className="w-3.5 h-3.5 text-purple-400"/>, bg:'bg-purple-500/20', label:'Video chờ xét duyệt', value:42, trend:'↓ 5%', trendColor:'text-red-400' },
              { icon: <Database className="w-3.5 h-3.5 text-blue-400"/>, bg:'bg-blue-500/20', label:'Lưu trữ đã dùng', value:`${stats?.totalStorageGB??0} GB`, trend:'↑ 100%', trendColor:'text-green-400' },
            ].map((item, i) => (
              <div key={i} className="bg-[#1a1c23] rounded-lg p-2.5 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className={`text-[9px] ${item.trendColor}`}>{item.trend}</p>
                </div>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_4.5fr] gap-3 mb-3">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={chartData?.revenue} cx="50%" cy="50%" innerRadius={45} outerRadius={68} paddingAngle={2} dataKey="value" stroke="none">
                  {chartData?.revenue?.map((_,index) => <Cell key={`c-${index}`} fill={PIE_COLORS[index%PIE_COLORS.length]}/>)}
                </Pie>
                <Tooltip contentStyle={{ backgroundColor:'#1F2937', borderColor:'#374151', borderRadius:'8px', color:'#fff', fontSize:'11px' }}/>
              </PieChart>
            </ResponsiveContainer>
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-sm font-bold text-white">{(stats?.monthlyRevenue??0).toLocaleString('vi-VN')}₫</span>
            ...
            {chartData?.revenue?.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-gray-400 truncate max-w-[85px]">{item.name}</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] font-semibold text-white">{stats?.monthlyRevenue ? ((item.value/stats.monthlyRevenue)*100).toFixed(0) : 0}%</span>
                  <p className="text-[9px] text-gray-500">{item.value?.toLocaleString('vi-VN')}₫</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#141418] p-4 rounded-xl border border-white/5">
          <h3 className="text-xs font-semibold text-white mb-3">Top danh mục <span className="text-gray-500 font-normal">(theo lượt xem)</span></h3>
          <div className="flex flex-col gap-3">
              ...
              return (
                <div key={idx}>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[11px] text-gray-300">{cat.name}</span>
                    <span className="text-[11px] text-gray-400 font-semibold">{cat.value?.toLocaleString()}</span>
                  </div>
        ...
        {/* Báo cáo chờ duyệt */}
        <div className="bg-[#141418] p-4 rounded-xl border border-white/5">
          <h3 className="text-xs font-semibold text-white mb-3">Báo cáo chờ duyệt</h3>
          <table className="w-full text-left table-fixed">
            <thead className="text-[9px] uppercase text-gray-500 border-b border-white/10">
              <tr>
                <th className="pb-2 pr-2 font-medium w-[22%]">Người báo cáo</th>
                <th className="pb-2 px-2 font-medium w-[32%]">Lý do</th>
                <th className="pb-2 px-2 font-medium w-[18%]">Thời gian</th>
                <th className="pb-2 px-2 font-medium text-center w-[14%]">Trạng thái</th>
            ...
            <tbody className="divide-y divide-white/5">
              {stats?.recentReports?.map((report, idx) => (
                <tr key={idx} className="group hover:bg-white/[0.02] transition-colors">
                  <td className="py-2 pr-2 text-[11px] text-gray-300 font-medium truncate">{report.user}</td>
                  <td className="py-1 px-1 text-[10px] text-red-400 line-clamp-2">{report.reason}</td>
                  <td className="py-2 px-2 text-[10px] text-gray-500 whitespace-nowrap">{report.time}</td>
                  <td className="py-2 px-2 text-center">
                    <span className={`inline-block px-1.5 py-0.5 rounded-full text-[9px] font-medium ${report.status==='Pending' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 'bg-green-500/10 text-green-400 border border-green-500/20'}`}>
                      {report.status==='Pending' ? 'Mới' : report.status}
                    </span>
                  </td>
                  <td className="py-2 pl-2 text-right">
                    <button className="text-[10px] px-2 py-0.5 rounded-md font-medium bg-indigo-600 hover:bg-indigo-500 text-white transition-colors">Xem</button>
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            {stats?.recentTransactions?.map((trx, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center justify-between pb-2.5 border-b border-white/5 last:border-0 last:pb-0">
                  ...
                  <div className="text-right">
                    <p className="text-[11px] font-bold text-white">{(trx.amount??0).toLocaleString('vi-VN')}₫</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1.4fr_1.4fr] gap-3">
        ...
        <div className="bg-[#141418] p-4 rounded-xl border border-white/5">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xs font-semibold text-white">Top video thịnh hành</h3>
            <span className="text-[11px] text-purple-400 cursor-pointer hover:underline">Xem tất cả →</span>
          </div>
            ...
            {stats?.topChannels?.map((channel, idx) => (
              <div key={idx} className="flex items-center gap-2.5 group">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-[11px] font-semibold text-gray-200 truncate group-hover:text-white transition-colors">{channel.channelName}</h4>
