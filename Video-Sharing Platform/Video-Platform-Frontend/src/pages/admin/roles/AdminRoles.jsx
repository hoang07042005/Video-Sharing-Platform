...
export default function AdminRoles() {
  const getRoleCardStyle = (role) => {
    if (role.color?.startsWith("#")) {
      return {
        className:
          "p-4 rounded-xl border relative overflow-hidden group cursor-pointer transition-all hover:brightness-110",
        style: {
  ...
  const normalizeRoleName = (value) => {
    ...
    const raw =
      typeof value === "string"
        ? value
        : value.name || value.Name || value.label || value.Label || "";
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      rolesData.forEach((role) => {
        const keys = [role.name, role.Name, role.label, role.Label]
          .filter(Boolean)
          .map((item) => String(item).trim().toLowerCase());
        keys.forEach((key) =>
          canonicalRoleMap.set(
            key,
            role.name || role.Name || role.label || role.Label,
          ),
        );
      });
      ...
      allPerms.forEach((perm) => {
        ...
        if (
          (pLower.includes("video") &&
            !pLower.includes("quản lý") &&
            !pLower.includes("ẩn")) ||
          ...
          pLower.includes("video") ||
          pLower.includes("danh mục") ||
          pLower.includes("từ khóa") ||
          (pLower.includes("bình luận") && pLower.includes("quản lý"))
        ) {
          targetGroup = 1;
        }
      ...
      setDynamicPermissionGroups(
        groups.filter((g) => g.permissions.length > 0),
      );
    } catch (error) {
  ...
  const handleSaveRole = async () => {
    ...
    try {
      ...
      showNotification(
        error.response?.data?.message || "Lỗi khi lưu vai trò!",
        "error",
      );
    } finally {
  ...
  const handleDeleteRole = async (id) => {
    ...
    try {
      ...
      showNotification(
        error.response?.data?.message || "Lỗi khi xóa vai trò!",
        "error",
      );
    }
  };
  ...
  const totalPermissions = dynamicPermissionGroups.reduce(
    (acc, g) => acc + g.permissions.length,
    0,
  );
  const totalActiveUsers = usersList.length;
  const usersWithRoles = usersList.filter(
    (u) => u.roles && u.roles.length > 0,
  ).length;
  const permissionCoverage =
    totalActiveUsers > 0
      ? Math.round((usersWithRoles / totalActiveUsers) * 100)
      : 0;
  ...
  const displayedGroups =
    selectedGroupFilter === "Tất cả nhóm quyền"
  ...
  return (
    <div className="p-4 max-w-[1600px] mx-auto min-h-full pb-20">
      ...
      {notification && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl border text-sm font-semibold animate-fade-in ${
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Tạo vai trò, phân quyền chi tiết và kiểm soát hoạt động hệ thống
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-[#141418] p-5 rounded-2xl border border-white/5 flex items-center gap-4 relative overflow-hidden group">
          ...
          <div>
            ...
            <div className="text-sm text-gray-200 font-semibold mt-0.5">
              Vai trò
            </div>
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="border-b border-white/10 bg-[#141418]/50 divide-x divide-white/10">
                    <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider w-[280px]">
                      NHÓM QUYỀN / QUYỀN HẠN
                    </th>
                    {rolesList.map((role) => {
                      const Icon = role.Icon;
                      return (
                  ...
                  {displayedGroups.map((group) => (
                    <Fragment key={group.name}>
                      ...
                      {group.permissions.map((perm) => (
                        <tr
                          key={perm.id}
                          className="hover:bg-white/[0.01] transition-colors divide-x divide-white/10"
                        >
            ...
            <div className="p-5 flex items-center justify-between border-b border-white/5">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Gán nhanh quyền cho người dùng
                </p>
              </div>
            </div>
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <div className="text-xs font-semibold text-gray-200 truncate max-w-[120px]">
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          value={
                            roleUpdates[user.id] || user.roles?.[0] || "User"
                          }
                          onChange={(e) =>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          ...
                          className="text-purple-400 hover:text-purple-300 text-xs font-semibold mr-4 transition-colors cursor-pointer"
                        >
                          Chỉnh sửa
                        </button>
                        <button
                          onClick={() => handleDeleteRole(role.id)}
                          className="text-red-400 hover:text-red-300 text-xs font-semibold transition-colors cursor-pointer"
                        >
                          Xóa
                        </button>
                      </td>
            ...
            <div className="p-5 border-b border-white/5 flex items-center justify-between bg-[#141418]">
              <div>
                <h3 className="text-lg font-bold text-white">
                  {modalRoleData.id ? "Chỉnh sửa vai trò" : "Thêm vai trò mới"}
                </h3>
                <p className="text-xs text-gray-500 mt-1">
                  Cấu hình thông tin và phân quyền chi tiết cho vai trò
                </p>
              </div>
              <button
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-5">
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-gray-500">
                      Mã duy nhất, không dấu, không khoảng trắng.
                    </p>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                ...
                <div className="flex gap-10">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative w-10 h-10 rounded-lg overflow-hidden border border-white/20 shrink-0 shadow-lg">
                        <input
                          type="color"
                          value={
                            modalRoleData.color?.startsWith("#")
                              ? modalRoleData.color
                              : "#a855f7"
                          }
                          onChange={(e) =>
                  ...
                  <div className="space-y-2 flex-1">
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                      Biểu tượng (Icon Lucide)
                    </label>
                    <div className="flex items-center gap-3">
                      <div {...getRoleIconBoxStyle(modalRoleData)}>
                        {(() => {
                          const IconComp =
                            LucideIcons[modalRoleData.icon] ||
                            LucideIcons.Users;
                ...
                <div className="pt-4 border-t border-white/5 space-y-4">
                  <h4 className="text-sm font-bold text-white flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-purple-400" /> Phân
                    quyền chi tiết
                  </h4>
                  ...
                  <div className="space-y-4">
                    {dynamicPermissionGroups.map((group) => (
                      <div
                        key={group.name}
                        className="bg-[#141418] rounded-xl p-4 border border-white/5"
                      >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {group.permissions.map((perm) => {
                            const isChecked =
                              modalRoleData.permissions.includes(perm.id);
                            return (
                              <label
                                key={perm.id}
                                className="flex items-center gap-3 cursor-pointer group/perm"
                              >
                                <div
                                ...
                                <span
                                  className={`text-xs ${isChecked ? "text-gray-200 font-medium" : "text-gray-500 group-hover/perm:text-gray-300"}`}
                                >
                                  {perm.label}
                                </span>
                                <input
