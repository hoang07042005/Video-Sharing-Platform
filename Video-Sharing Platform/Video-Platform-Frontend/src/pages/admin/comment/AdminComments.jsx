...
const AdminComments = () => {
    ...
    try {
      ...
      if (modalMode === "add") {
        const res = await axios.post(
          "/api/admincomments/bannedwords",
          currentKeyword,
        );
        setKeywords([res.data, ...keywords]);
      } else {
        const res = await axios.put(
          `/api/admincomments/bannedwords/${currentKeyword.id}`,
          currentKeyword,
        );
        setKeywords(
          keywords.map((k) => (k.id === currentKeyword.id ? res.data : k)),
        );
      }
  ...
  const approvedCount = comments.filter(
    (c) => c.filterStatus === "Normal",
  ).length;
  const pendingCount = comments.filter(
    (c) => c.filterStatus === "Warning",
  ).length;
  const rejectedCount = comments.filter(
    (c) => c.filterStatus === "Filtered" || c.filterStatus === "Blocked",
  ).length;
  const reportedCount = comments.filter(
    (c) => c.filterStatus === "Reported",
  ).length;
  ...
  return (
    <div className="p-2 md:p-2 max-w-[1600px] mx-auto min-h-full">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 mb-8 gap-4">
        <div className="flex overflow-x-auto scrollbar-hide gap-8">
          <button
            ...
            className={`pb-3 flex items-center gap-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === "all" ? "border-purple-500 text-purple-400" : "border-transparent text-gray-400 hover:text-white"}`}
          >
            <MessageSquare className="w-4 h-4" /> Tất cả bình luận
          </button>
          <button
            onClick={() => setActiveTab("pending")}
            className={`pb-3 flex items-center gap-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === "pending" ? "border-purple-500 text-purple-400" : "border-transparent text-gray-400 hover:text-white"}`}
          >
            <Clock className="w-4 h-4" /> Chờ duyệt
          </button>
          <button
            onClick={() => setActiveTab("approved")}
            className={`pb-3 flex items-center gap-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === "approved" ? "border-purple-500 text-purple-400" : "border-transparent text-gray-400 hover:text-white"}`}
          >
            <CheckCircle className="w-4 h-4" /> Đã duyệt
          </button>
          <button
            onClick={() => setActiveTab("rejected")}
            className={`pb-3 flex items-center gap-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === "rejected" ? "border-purple-500 text-purple-400" : "border-transparent text-gray-400 hover:text-white"}`}
          >
            <XCircle className="w-4 h-4" /> Bị từ chối / Ẩn
          </button>
          <button
            onClick={() => setActiveTab("reported")}
            className={`pb-3 flex items-center gap-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === "reported" ? "border-purple-500 text-purple-400" : "border-transparent text-gray-400 hover:text-white"}`}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div className="bg-[#141418] rounded-2xl border border-white/5 p-6 flex flex-col justify-between h-[150px]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1 text-[10px] text-emerald-400 font-medium">
              <LucideIcons.TrendingUp className="w-3 h-3" /> 12.5%{" "}
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 p-6 flex flex-col justify-between h-[150px]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1 text-[10px] text-emerald-400 font-medium">
              <LucideIcons.TrendingUp className="w-3 h-3" /> 8.7%{" "}
              <span className="text-gray-500">so với tuần trước</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 p-6 flex flex-col justify-between h-[150px]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1 text-[10px] text-emerald-400 font-medium">
              <LucideIcons.TrendingUp className="w-3 h-3" /> 3.2%{" "}
              <span className="text-gray-500">so với tuần trước</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 p-6 flex flex-col justify-between h-[150px]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1 text-[10px] text-red-400 font-medium">
              <LucideIcons.TrendingDown className="w-3 h-3" /> 4.1%{" "}
              <span className="text-gray-500">so với tuần trước</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#141418] rounded-2xl border border-white/5 p-6 flex flex-col justify-between h-[150px]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1 text-[10px] text-emerald-400 font-medium">
              <LucideIcons.TrendingUp className="w-3 h-3" /> 10.3%{" "}
              <span className="text-gray-500">so với tuần trước</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col xl:flex-row gap-3 items-stretch">
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full">
          {/* Main Table */}
          <div className="bg-[#141418] rounded-2xl border border-white/5 overflow-hidden flex flex-col flex-1">
                ...
                <thead>
                  <tr className="border-b border-white/5 bg-[#1C1C24]/50 text-[10px] uppercase tracking-wider text-gray-400 font-semibold">
                    <th className="px-5 py-4 w-10">
                      <input
                        type="checkbox"
                        className="rounded border-white/20 bg-transparent text-purple-600 focus:ring-purple-600/50 cursor-pointer"
                      />
                    </th>
                          ...
                          <td className="px-1 py-2">
                            ...
                            <div className="flex items-center gap-2 text-[10px] text-gray-500 font-medium">
                              <span className="flex items-center gap-1">
                                <LucideIcons.ThumbsUp className="w-3.5 h-3.5" />{" "}
                                0
                              </span>
                              <span className="flex items-center gap-1">
                              ...
                              {(c.filterStatus === "Filtered" ||
                                c.filterStatus === "Blocked") && (
                                <span className="flex items-center gap-1 text-red-400 px-0 py-0 rounded">
                                  <ShieldAlert className="w-3 h-3" /> Bị báo cáo
                                </span>
                              )}
                            </div>
                          </td>
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-20 h-14 rounded-lg overflow-hidden shrink-0 bg-gray-800 relative">
                                {c.videoThumbnail ? (
                                  ...
                                  <div className="w-full h-full bg-purple-900/40 flex items-center justify-center text-purple-500">
                              ...
                              <div className="relative group/menu">
                                ...
                                <div className="absolute right-0 top-full mt-1 w-32 bg-[#1C1C24] border border-white/10 rounded-lg shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10 py-1">
                                  <button
                                    ...
                                    className="w-full text-left px-4 py-2 text-xs font-medium text-red-400 hover:bg-white/5 flex items-center gap-2 cursor-pointer"
                                  >
                                    <Trash2 className="w-3.5 h-3.5" /> Xóa bình
                                    luận
                                  </button>
                                </div>
                              </div>
            ...
            <div className="p-4 border-t border-white/5 flex items-center justify-between text-sm mt-auto">
              ...
              <div className="flex items-center gap-4">
                <span className="text-gray-500 text-xs hidden sm:block">
                  Hiển thị {filteredComments.length === 0 ? 0 : startIndex + 1}{" "}
                  -{" "}
        ...
        <div className="w-full xl:w-[380px] shrink-0 h-full">
          <div className="bg-[#141418] rounded-2xl border border-white/5 overflow-hidden flex flex-col h-full">
            <div className="p-5 border-b border-white/5 flex items-center justify-between">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                Từ khóa bị cấm{" "}
                <LucideIcons.Sparkles className="w-4 h-4 text-gray-500" />
              </h2>
            ...
            <div className="p-4 border-b border-white/5 flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full bg-[#1C1C24] border border-white/5 rounded-xl pl-8 pr-3 py-2 text-xs text-white focus:outline-none focus:border-purple-500 transition-colors"
                />
                <Search className="w-3.5 h-3.5 text-gray-500 absolute left-3 top-2.5" />
              </div>
              <button
                onClick={openAddKeywordModal}
                className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-[#FF5722] to-[#CE1414FA] text-white text-xs font-semibold rounded-xl transition-colors shadow-lg shadow-[#FF5722]/20 cursor-pointer"
              >
                  ...
                  {loadingKeywords ? (
                    ...
                    keywords.slice(0, 10).map((kw) => (
                      <tr
                        ...
                        className="hover:bg-white/[0.02] transition-colors group"
                      >
                        <td className="px-4 py-1 font-medium text-gray-200 truncate max-w-[100px]">
                          {kw.keyword}
                        </td>
                        <td className=" text-center">
                          <span
                            className={`font-semibold ${kw.level === "High" ? "text-red-400" : kw.level === "Medium" ? "text-yellow-400" : "text-blue-400"}`}
                          >
                            {kw.level === "High"
                              ? "Cao"
                              : kw.level === "Medium"
                        ...
                        <td className="px-1 py-1 text-center">
                          <span
                            className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-md ${kw.isActive ? "text-emerald-400" : "text-gray-400"}`}
                          >
          ...
          <div className="bg-[#1C1C24] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h2 className="text-white font-bold text-lg">
                {modalMode === "add" ? "Thêm Từ khóa" : "Chỉnh sửa Từ khóa"}
              </h2>
              <button
            ...
            <div className="p-5 space-y-5">
              <div>
                <label className="block text-xs text-gray-400 font-medium mb-1.5 uppercase tracking-wider">
                  Từ khóa bị cấm <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["Low", "Medium", "High"].map((level) => (
                    <button
                      key={level}
                      onClick={() =>
                        setCurrentKeyword({ ...currentKeyword, level })
                      }
            ...
            <div className="flex gap-3 p-5 border-t border-white/10 bg-bg-[#0F0F0F] rounded-b-2xl">
              <button
                onClick={() => setIsModalOpen(false)}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-gray-400 bg-white/5 hover:bg-white/10 transition-colors cursor-pointer"
              >
                Hủy
              </button>
              <button
                onClick={handleSaveKeyword}
                disabled={isSaving || !currentKeyword.keyword.trim()}
