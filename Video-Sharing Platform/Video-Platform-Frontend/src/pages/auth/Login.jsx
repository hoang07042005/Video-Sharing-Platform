...
export default function Login() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        if (res.data) {
          // Must handle both boolean (true/false) and string ("true"/"false") from DB
          const parseBool = (val) => val === true || val === "true";
          if (res.data.googleLoginEnabled !== undefined)
      ...
      .catch(() => {
        // fallback: keep both enabled if API fails
      });
  }, []);
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      const { token, roles, handle, avatarUrl, userId, isNewUser } =
        response.data;
        ...
        <div className="w-full max-w-md mx-auto md:mx-0">
          ...
          <div className="flex items-center gap-6 mb-10">
            ...
            <h2 className="text-[23px] font-medium text-white tracking-tight">
              Đăng nhập{" "}
              <span className="text-[#FF5722] font-semibold">để tiếp tục</span>
            </h2>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Link
                  to="/forgot-password"
                  className="text-xs text-[#FF8A65] hover:text-white transition-colors"
                >
                  Quên mật khẩu?
                </Link>
              </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            ...
            <div className="relative group">
              <button
                type="button"
                onClick={() => googleEnabled && googleLogin()}
                ...
                className={`flex items-center justify-center gap-3 py-2.5 bg-transparent border rounded-xl transition-all w-full
                  ${
                    googleEnabled
              ...
              {!googleEnabled && (
                <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-black/80 text-gray-300 text-[10px] px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  Không khả dụng
                </div>
              )}
            </div>
            ...
            <div className="relative group">
              <button
                ...
                title={
                  !facebookEnabled
                    ? "Đăng nhập Facebook hiện không khả dụng"
              ...
              >
                ...
                >
                  ...
                  <path
                    fill="#ffffff"
                    d="M16.671 15.542l.532-3.469h-3.328V9.823c0-.949.465-1.874 1.956-1.874h1.514V5.008s-1.374-.235-2.686-.235c-2.741 0-4.533 1.662-4.533 4.669v2.63H7.078v3.469h3.047v8.385a12.09 12.09 0 003.75 0v-8.385h2.796z"
                  />
                </svg>
                <span className="font-medium text-sm text-gray-300">
                  Facebook
        ...
        <div className="absolute bottom-16 left-12 right-12 z-20">
          <div className="bg-black/30 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-2xl">
            ...
            <p className="text-gray-300 text-sm lg:text-base max-w-md leading-relaxed font-light">
              Trải nghiệm nền tảng phát trực tiếp đẳng cấp với chất lượng hình
              ảnh tuyệt đỉnh và cộng đồng sôi động nhất.
            </p>
          </div>
        </div>
      </div>
