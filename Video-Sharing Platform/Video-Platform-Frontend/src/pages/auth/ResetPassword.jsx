...
export default function ResetPassword() {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Có lỗi xảy ra. Vui lòng thử lại hoặc yêu cầu mã OTP mới.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full max-w-md mx-auto md:mx-0">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">
              Mã xác thực đã được gửi đến <span className="text-white font-medium">{email || 'email của bạn'}</span>.
            </p>
          </div>
          ...
          <form onSubmit={handleResetPassword} className="space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-sm text-gray-300">Nhập lại mật khẩu mới</label>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 w-5 h-5" />
                <input 
                  ...
                  onPaste={(e) => {
                    e.preventDefault();
                    setError('Vui lòng tự nhập lại mật khẩu, không sử dụng copy-paste.');
                  }}
                  required
                />
                <button 
        ...
        <div className="absolute bottom-16 left-12 right-12 z-20">
          <div className="bg-black/30 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-2xl">
            ...
            <p className="text-gray-300 text-sm lg:text-base max-w-md leading-relaxed font-light">
              Mật khẩu mạnh mẽ giúp bảo vệ tài khoản và mọi dữ liệu quan trọng của bạn trên nền tảng.
            </p>
          </div>
        </div>
      </div>
