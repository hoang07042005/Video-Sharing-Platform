...
function ShortItem({
  ...
  useEffect(() => {
    if (showComments && comments.length === 0) {
      ...
      axios
        .get(`/api/videos/${short.id}/comments`)
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (!viewRecorded.current) {
        ...
        axios
          .post(`/api/videos/${short.id}/record-view`, {}, { headers })
          .catch(console.error);
      }
    } else {
      video.pause();
  ...
  const handleFollow = async (e) => {
    ...
    try {
      await axios.post(
        `/api/channels/${short.channelId}/follow`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
    } catch (err) {
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        setBanModal({
          message:
            error.response?.data?.message || "Kênh của bạn đã bị đình chỉ.",
        });
      }
    }
  };
  ...
  const isOwnChannel = (() => {
    ...
    return (
      Boolean(currentUserId && short.ownerUserId) &&
      String(currentUserId).toLowerCase() ===
        String(short.ownerUserId).toLowerCase()
    );
  })();
  ...
  const renderReplyInput = (targetId, parentId) => {
    ...
    return (
      <div
        className={`flex gap-3 mt-3 w-full ${targetId === parentId ? "ml-11" : ""}`}
      >
        ...
        <div className="flex-1 flex flex-col gap-2 relative z-10">
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="px-3 py-1 hover:bg-[#3F3F3F] rounded-full text-white text-[13px] font-medium transition-colors cursor-pointer"
            >
              Hủy
            </button>
            <button
              onClick={() => handleReply(parentId)}
              className={`px-3 py-1 rounded-full text-[#0F0F0F] text-[13px] font-medium transition-colors ${replyText.trim() ? "bg-[#3EA6FF] hover:bg-[#65B8FF] cursor-pointer" : "bg-[#272727] text-gray-500"}`}
            >
              Phản hồi
            </button>
          </div>
          ...
          {replies.map((reply, index) => {
            ...
            return (
              <div key={reply.id} className="flex flex-col relative z-10">
                {index === 0 && (
                  <div className="absolute top-[-16px] left-[-20px] w-[1px] h-[16px] bg-[#FF5722] z-0"></div>
                )}
                <div className="absolute top-0 left-[-20px] w-[20px] h-[16px] border-l-[1.5px] border-b-[1.5px] border-[#FF5722] rounded-bl-[12px] z-0"></div>
                {!isLast && (
                  <div className="absolute top-[16px] bottom-[-12px] left-[-20px] w-[1.5px] bg-[#FF5722] z-0"></div>
                )}
                ...
                <div className="flex gap-2 relative z-10">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      <button
                        onClick={() =>
                          handleReplyLike(commentId, reply.id, false)
                        }
                      ...
                      <button
                        ...
                        className="text-white/70 hover:text-white text-[11px] font-medium cursor-pointer ml-1"
                      >
                        Phản hồi
                      </button>
                    </div>
  ...
  return (
    <div
      className={`w-full h-full flex items-center justify-center select-none relative overflow-hidden transition-all duration-300 ease-in-out ${showComments ? "pr-[340px]" : "pr-0"}`}
    >
      {/* Ban Modal */}
      {banModal && (
      ...
      <div className="flex items-end gap-5 w-full max-w-[1400px] justify-center px-2 sm:px-4">
        <div className="flex flex-col gap-3 flex-1 min-w-[150px] max-w-[360px] pb-2">
          <div className="flex items-center gap-2 flex-wrap">
            <Link
              to={`/c/${short.channelHandle}`}
              className="flex items-center gap-2 group"
            >
              <div className="w-8 h-8 rounded-full overflow-hidden border border-white/20 flex-shrink-0">
                <img
                  src={
                    short.channelAvatarUrl ||
                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${short.channelName}`
                  }
          ...
          {short.description && (
            <div className="relative">
              ...
              {(short.description.length > 40 ||
                short.description.includes("\n")) && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (isExpanded) {
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-1">
              <span className="text-white/70 text-[10px] font-mono tabular-nums">
        ...
        <div className="flex-1 min-w-[150px] max-w-[360px] pb-8 flex flex-col items-start">
          <div className="flex flex-col gap-3 items-start">
            ...
            >
              ...
              >
                <Heart
                  className={`w-5 h-5 transition-all duration-200 ${liked ? "fill-red-500 text-red-500 scale-110" : "text-white"}`}
                />
              </div>
            ...
            >
              ...
              >
                <MessageCircle
                  className={`w-5 h-5 transition-all duration-200 ${showComments ? "text-white fill-white" : "text-white"}`}
                />
              </div>
              <span className="text-white/80 text-[11px] font-semibold">
              ...
              onClick={(e) => {
                ...
                if (!token)
                  return onRequestLogin(
                    "Vui lòng đăng nhập để chia sẻ video này",
                  );
            ...
            >
              ...
              >
                <Bookmark
                  className={`w-5 h-5 transition-all duration-200 ${saved ? "fill-yellow-400 text-yellow-400" : "text-white"}`}
                />
              </div>
              <span className="text-white/80 text-[11px] font-semibold">
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 pb-3">
          ...
          <div className="flex items-center gap-3">
            <button className="text-white hover:text-white/70 transition-colors p-1">
            ...
            comments.map((c) => (
              <div key={c.id} className="flex flex-col gap-2">
                <div className="flex gap-3">
                  <div className="w-8 h-8 rounded-full overflow-hidden shrink-0">
                    <img
                      src={
                        c.avatarUrl ||
                        `https://api.dicebear.com/7.x/avataaars/svg?seed=${c.userId}`
                      }
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-white text-[14px] leading-relaxed mb-2 pr-4">
                      {c.content}
                    </p>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        ...
                        className="text-white/80 text-[12px] font-semibold hover:text-white ml-2 transition-colors"
                      >
                        Phản hồi
                      </button>
                    </div>
                  </div>
                  <button className="text-white/60 hover:text-white h-fit mt-1">
                ...
                {expandedReplies[c.id] &&
                  renderReplyTree(buildReplyTree(c.replies), c.id)}

                {c.replies?.length > 0 && (
                  <div className="mt-1 ml-[36px]">
                    <button
                      onClick={() => toggleReplies(c.id)}
                      className="flex items-center gap-2 text-[#3ea6ff] hover:bg-[#3ea6ff]/10 px-3 py-1.5 rounded-full text-xs font-medium transition-colors w-fit cursor-pointer"
                    >
                      {expandedReplies[c.id] ? (
          ...
          <div className="flex-1 border-b border-white/20 pb-1.5 flex items-center gap-2 group focus-within:border-white transition-colors">
            ...
            {newComment.trim() && (
              <button
                ...
                className="text-[#3ea6ff] hover:text-[#5eb7ff] transition-colors p-1"
              >
...
export default function Shorts() {
    ...
    const fetchShorts = async () => {
      try {
        ...
        let data = res.data.map((short) => {
          if (
            !short.videoUrl ||
            short.videoUrl.includes("example.com") ||
            short.videoUrl.includes("commondatastorage")
          ) {
  ...
  if (loading)
    return (
      <div className="w-full h-[calc(100vh-64px)] flex items-center justify-center bg-[#0F0F0F]">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white"></div>
      </div>
    );
  if (shorts.length === 0)
    return (
      <div className="w-full h-[calc(100vh-64px)] flex items-center justify-center bg-[#0F0F0F] text-white">
  ...
  return (
    <>
        ...
        {shorts.map((short, i) => (
          ...
          >
            <ShortItem
              ...
              onRequestLogin={(msg) =>
                setLoginModal({ isOpen: true, message: msg })
              }
            />
          </div>
        ))}
      ...
      <div
        className={`fixed top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3 pointer-events-none transition-all duration-300 ease-in-out ${globalShowComments ? "right-[364px]" : "right-6"}`}
      >
        <button
          onClick={goPrev}
