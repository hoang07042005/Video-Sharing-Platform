    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#212121] rounded-2xl max-w-md w-full border border-white/10 shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            <h3 className="text-xl font-bold text-white mb-2">Xóa video này?</h3>
        ...
        <div className="flex gap-3 p-2 rounded-lg">
          <div className="w-24 h-14 rounded overflow-hidden shrink-0 bg-black/50">
            <img src={video.thumbnailUrl || 'https://via.placeholder.com/600x400?text=No+Thumbnail'} alt={video.title} className="w-full h-full object-cover" />
          </div>
          <strong className="text-white text-sm line-clamp-2 mt-1">{video.title}</strong>
        </div>
        <br/>
        ...
        <div className="mb-6 flex items-start gap-3">
          ...
          />
          <label htmlFor="confirmDelete" className="text-gray-300 text-sm cursor-pointer select-none">
            Tôi đã hiểu rằng hành động này không thể hoàn tác và xác nhận xóa video.
          </label>
        </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0F0F0F] pb-20">
        ...
        <div className="relative z-10 w-full max-w-[1280px] mx-auto px-4 md:px-8 pt-32 md:pt-40 pb-0 flex flex-col">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8 w-full">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 w-full lg:w-auto">
              ...
              <div className="flex flex-col items-start gap-1">
                ...
                >
                  ...
                  <span className="text-white font-medium ml-1 whitespace-nowrap">...xem thêm</span>
                </div>
                ...
                {channel.links && channel.links.length > 0 && (
                  <div className="flex items-center flex-wrap gap-4 mt-2 mb-3">
                    {channel.links.map((link, idx) => (
                      <a key={idx} href={link.url.startsWith('http') ? link.url : `https://${link.url}`} target="_blank" rel="noreferrer" className="text-[#3EA6FF] text-xs md:text-sm font-medium hover:underline flex items-center gap-1.5 bg-[#1A1A1A]/40 px-3 py-1.5 rounded-full border border-white/5">
                        <LinkIcon className="w-3.5 h-3.5" />
                        {link.title || link.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0]}
                      </a>
                    ))}
                  </div>
                )}
                ...
                <div className="flex items-center flex-wrap gap-3">
                  {isOwner ? (
                    ...
                    <button
                      ...
                      className={`px-5 py-2 rounded-full font-semibold text-sm transition-all flex items-center gap-1.5 ${isSubscribed
            ...
            <div className="bg-[#1a1a1a]/40 backdrop-blur-md border border-white/10 rounded-2xl p-5 lg:p-6 flex items-center justify-between gap-4 md:gap-8 shadow-2xl shrink-0 lg:ml-4 w-full lg:w-auto overflow-x-auto mt-6 lg:mt-0">
              ...
              <div className="flex flex-col items-center min-w-[60px] md:min-w-[75px]">
                <span className="text-white font-bold text-xl md:text-2xl">{formatViews(channel.subscriberCount)}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-8 mt-8">
        ...
        {(() => {
          if (activeTab === 'overview') {
            ...
            const featuredVideo = normalVideos.length > 0 ? normalVideos[0] : null;
            ...
            return (
              <div className="space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4">
                  ...
                  {featuredVideo ? (
                    ...
                    >
                      ...
                      <div className="relative w-full md:w-[45%] lg:w-[50%] aspect-video bg-black rounded-xl overflow-hidden shrink-0">
                        <img
                          src={featuredVideo.thumbnailUrl || 'https://via.placeholder.com/1280x720'}
                      ...
                      <div className="flex flex-col flex-1 py-2 justify-center">
                        ...
                        <div className="flex items-center text-[13px] text-gray-400 mb-3 gap-1.5">
                          ...
                          <span>•</span>
                          <span>{formatViews(featuredVideo.viewsCount)} lượt xem</span>
                          <span>•</span>
                        ...
                        <div className="flex flex-wrap gap-2 mb-5">
                          <span className="bg-[#1A1A1A] border border-[#2A2A2A] text-[#FF4E00] px-3 py-1 text-[12px] font-medium rounded-full">#NodeJS</span>
                          <span className="bg-[#1A1A1A] border border-[#2A2A2A] text-gray-400 px-3 py-1 text-[12px] font-medium rounded-full">#JavaScript</span>
                          <span className="bg-[#1A1A1A] border border-[#2A2A2A] text-gray-400 px-3 py-1 text-[12px] font-medium rounded-full">#Setup</span>
                        </div>
                  ...
                  <div className="bg-[#121212] border border-[#2A2A2A] rounded-2xl p-5 h-full flex flex-col">
                    <div className="flex items-center gap-2 text-white font-bold mb-5 text-[16px]">
                      <span className="text-[#FF4E00] font-black text-lg">{'<'}</span> Nổi bật
                    </div>
                    ...
                    <div className="space-y-4 flex-1">
                      {featuredList.map(v => (
                        <Link to={v.isShort ? `/shorts?id=${v.id}` : `/watch/${v.id}`} key={v.id} className="flex gap-3 group cursor-pointer items-center">
                          ...
                          <div className="relative w-32 aspect-video rounded-lg overflow-hidden shrink-0">
                            <img
                              src={v.thumbnailUrl || 'https://via.placeholder.com/600x400'}
                              alt={v.title}
                              className="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                            />
                      ...
                      {featuredList.length === 0 && <div className="text-sm text-gray-500">Chưa có video</div>}
                    </div>
                  </div>
                </div>
                ...
                {latestVideos.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={() => setActiveTab('videos')} className="text-sm text-gray-400 hover:text-white flex items-center transition-colors">Xem tất cả {'>'}</button>
                    </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2 text-white font-bold text-xl">
                      <span className="text-[#FF4E00]">❤</span> Danh sách phát nổi bật
                    </div>
                    <button className="text-sm text-gray-400 hover:text-white flex items-center transition-colors">Xem tất cả {'>'}</button>
                  </div>
                  {playlists.length > 0 ? (
                      ...
                      {playlists.map(playlist => (
                        <div key={playlist.id} className="group cursor-pointer">
                          <div className="relative aspect-video rounded-xl overflow-hidden mb-3 shadow-lg">
                            <img src={playlist.thumbnailUrl} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                          ...
                          <h4 className="text-white font-semibold text-xs line-clamp-2 leading-snug">{playlist.title}</h4>
                          <p className="text-gray-400 text-xs mt-1.5">{playlist.videoCount} video</p>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-gray-500 text-sm italic">Chưa có danh sách phát nào.</div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-[#1A1A1A]/40 border border-white/5 rounded-2xl p-5 shadow-lg flex flex-col">
                    <div className="flex items-center justify-between mb-5">
                      <div className="flex items-center gap-2 text-white font-bold text-lg">
                        <span className="text-[#FF4E00]">⚡</span> Shorts mới nhất
                      </div>
                      <button className="text-xs text-gray-400 hover:text-white flex items-center transition-colors">Xem tất cả {'>'}</button>
                    </div>
                    <div className="grid grid-cols-3 gap-3 relative flex-1">
                      ...
                      {shortVideos.length === 0 && <div className="col-span-3 flex items-center justify-center text-sm text-gray-500">Chưa có video ngắn</div>}
                    ...
                    <div className="space-y-4">
                      <div className="bg-[#2A2A2A]/50 border border-white/5 rounded-xl p-4">
                        <div className="flex gap-3 mb-3">
                          <img src={channel?.avatarUrl || 'https://via.placeholder.com/40'} className="w-9 h-9 rounded-full shrink-0" />
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-sm text-white font-medium">{channel?.channelName}</span>
                              <span className="text-[10px] text-gray-500">2 ngày trước</span>
                            </div>
                          </div>
                        </div>
                        <p className="text-xs md:text-sm text-gray-300 mb-4 leading-relaxed">Bạn đang gặp vấn đề gì trong lập trình?<br />Hãy để lại bình luận, mình sẽ giải đáp nhé! 👇</p>
                        <div className="flex items-center gap-5 text-gray-400 text-xs">
                          <button className="flex items-center gap-1.5 hover:text-white transition-colors">👍 512</button>
                          <button className="flex items-center gap-1.5 hover:text-white transition-colors">💬 128</button>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#1A1A1A]/40 border border-white/5 rounded-2xl p-5 shadow-lg">
                    <div className="flex items-center gap-2 text-white font-bold mb-5 text-lg">
                      <span className="text-[#FF4E00]">{'<'}</span> Kênh bạn nên xem
                    </div>
                    <div className="space-y-5">
                      {[
                        { name: 'F8 Official', subs: '1,2 Tr', avt: 'https://ui-avatars.com/api/?name=F8&background=random' },
                        { name: 'Vinh Xô', subs: '892 N', avt: 'https://ui-avatars.com/api/?name=VX&background=random' },
                        { name: 'Code With Harry', subs: '3,1 Tr', avt: 'https://ui-avatars.com/api/?name=CH&background=random' }
                      ].map((c, i) => (
                        <div key={i} className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <img src={c.avt} className="w-10 h-10 rounded-full shrink-0" />
                            <div>
                              <h4 className="text-white text-sm font-medium">{c.name}</h4>
                              <p className="text-gray-500 text-xs">{c.subs} đăng ký</p>
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 bg-[#1A1A1A]/30 border border-white/5 rounded-2xl p-6 shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#FF4E00]/10 flex items-center justify-center text-[#FF4E00] shrink-0 text-lg">⚡</div>
                    <div>
                      <h4 className="text-white text-sm font-semibold mb-0.5">Cập nhật liên tục</h4>
                      <p className="text-gray-500 text-[10px]">Nội dung mới mỗi tuần</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#FF4E00]/10 flex items-center justify-center text-[#FF4E00] shrink-0 font-bold">HD</div>
                    <div>
                      <h4 className="text-white text-sm font-semibold mb-0.5">Chất lượng cao</h4>
                      <p className="text-gray-500 text-[10px]">Hình ảnh & âm thanh tốt nhất</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#4FC3F7]/10 flex items-center justify-center text-[#4FC3F7] shrink-0 text-lg">📖</div>
                    <div>
                      <h4 className="text-white text-sm font-semibold mb-0.5">Kiến thức thực tế</h4>
                      <p className="text-gray-500 text-[10px]">Dễ hiểu & dễ áp dụng</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#FF5252]/10 flex items-center justify-center text-[#FF5252] shrink-0 text-lg">👥</div>
                    <div>
                      <h4 className="text-white text-sm font-semibold mb-0.5">Cộng đồng thân thiện</h4>
                      <p className="text-gray-500 text-[10px]">Học hỏi & chia sẻ</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    {isOwner && (
                      <Link to="/studio/upload" className="flex items-center gap-2 px-4 py-2 bg-[#FF4E00]/10 hover:bg-[#FF4E00]/20 text-[#FF4E00] text-sm font-medium rounded-full transition-colors border border-[#FF4E00]/30">
                  ...
                  <div className="flex gap-2">
                    <button className="px-4 py-1.5 bg-white text-black text-sm font-medium rounded-full">Mới nhất</button>
                    <button className="px-4 py-1.5 bg-white/10 hover:bg-white/20 text-white text-sm font-medium rounded-full transition-colors">Phổ biến</button>
                  </div>
                </div>
                {filteredVideos.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 bg-[#1A1A1A]/50 rounded-2xl border border-white/5">
                    <p className="text-gray-400 mb-4">{searchQuery ? 'Không tìm thấy video nào phù hợp với tìm kiếm.' : 'Kênh này chưa có video nào.'}</p>
                    {isOwner && !searchQuery && (
                      <Link to="/studio/upload" className="flex items-center gap-2 px-6 py-2.5 bg-[#FF4E00] hover:bg-[#FF6A00] text-white font-medium rounded-full transition-colors">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-8">
                    {filteredVideos.map((video) => (
                      <VideoCard
                        key={video.id}
                        isOwner={isOwner}
                        onEdit={() => { setEditingVideo(video); setActiveTab('videos'); }}
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    {isOwner && (
                      <Link to="/studio/upload-short" className="flex items-center gap-2 px-4 py-2 bg-[#FF4E00]/10 hover:bg-[#FF4E00]/20 text-[#FF4E00] text-sm font-medium rounded-full transition-colors border border-[#FF4E00]/30">
                ...
                {filteredShorts.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 bg-[#1A1A1A]/50 rounded-2xl border border-white/5">
                    <p className="text-gray-400 mb-4">{searchQuery ? 'Không tìm thấy video ngắn nào phù hợp với tìm kiếm.' : 'Kênh này chưa có video ngắn nào.'}</p>
                    {isOwner && !searchQuery && (
                      <Link to="/studio/upload-short" className="flex items-center gap-2 px-6 py-2.5 bg-[#FF4E00] hover:bg-[#FF6A00] text-white font-medium rounded-full transition-colors">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filteredShorts.map((short) => (
                      <div key={short.id} className="relative group cursor-pointer">
                        <Link to={`/shorts?id=${short.id}`} className="relative aspect-[9/16] rounded-xl overflow-hidden shadow-md block bg-[#0a0a0a]">
                          <img src={short.thumbnailUrl || 'https://via.placeholder.com/300x500?text=No+Thumbnail'} alt=""
                            className="absolute inset-0 w-full h-full object-cover blur-xl opacity-40 group-hover:scale-105 transition-transform duration-500" />
                          <img src={short.thumbnailUrl || 'https://via.placeholder.com/300x500?text=No+Thumbnail'} alt={short.title}
                            className="absolute inset-0 w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" />
                          <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                            ...
                            <p className="text-gray-400 text-xs mt-1">{formatViews(short.viewsCount || 0)} lượt xem</p>
                          </div>
                        </Link>
                        {isOwner && (
                          <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setEditingVideo(short); setActiveTab('shorts'); }} className="p-2 bg-black/60 hover:bg-black/80 rounded-full text-white backdrop-blur-sm transition-colors">
                              <Pencil className="w-4 h-4" />
                            </button>
                            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setVideoToDelete(short); }} className="p-2 bg-red-500/80 hover:bg-red-500 rounded-full text-white backdrop-blur-sm transition-colors">
      ...
      <DeleteVideoModal 
        ...
        onSuccess={() => { setVideoToDelete(null); fetchChannelData(handle); }} 
      />
              ...
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  {isOwner && !isEditingLinks && (
                    <button onClick={() => { setEditLinksContent(channel.links || []); setIsEditingLinks(true); }} className="p-1 text-gray-400 hover:text-white hover:bg-white/10 rounded-full transition-colors">
                      <Pencil className="w-4 h-4" />
                    </button>
                  )}
                </div>
                ...
                {isEditingLinks ? (
                  <div className="space-y-3">
                    {editLinksContent.map((link, idx) => (
                      <div key={idx} className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                        ...
                        />
                        <button onClick={() => {
                          const newLinks = editLinksContent.filter((_, i) => i !== idx);
                          setEditLinksContent(newLinks);
                    ...
                    {editLinksContent.length < 3 && (
                      <button onClick={() => setEditLinksContent([...editLinksContent, { title: '', url: '' }])} className="text-[#3EA6FF] text-sm font-medium hover:underline flex items-center gap-1 mt-2">
                        + Thêm đường liên kết
                      </button>
                    )}
                    <div className="flex justify-end gap-2 mt-4 pt-2 border-t border-white/5">
                      <button onClick={() => setIsEditingLinks(false)} className="px-4 py-1.5 rounded-full text-sm font-medium text-white hover:bg-white/10 transition-colors">Hủy</button>
                      <button onClick={() => handleSaveProfile('links', editLinksContent, setIsEditingLinks)} className="px-4 py-1.5 rounded-full text-sm font-medium bg-[#3EA6FF] text-black hover:bg-[#65B8FF] transition-colors flex items-center gap-2">
                        {isSavingProfile ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : 'Lưu'}
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {(!channel.links || channel.links.length === 0) ? (
                      <div className="text-gray-400 text-sm">Chưa có đường liên kết nào.</div>
                    ) : (
                      channel.links.map((link, idx) => (
                        <a key={idx} href={link.url.startsWith('http') ? link.url : `https://${link.url}`} target="_blank" rel="noreferrer" className="flex items-start gap-4 text-gray-300 hover:bg-white/5 p-3 rounded-xl transition-colors">
                          <LinkIcon className="w-6 h-6 text-gray-400 shrink-0" />
                          <div>
                            <div className="font-medium text-white text-sm mb-1">{link.title || link.url}</div>
                            <div className="text-[#3EA6FF] text-sm truncate max-w-[200px] sm:max-w-xs md:max-w-md">{link.url}</div>
                          </div>
                        </a>
                      ))
                    )}
