...
const INSPIRATIONS = [
  {
    label: "Thư giãn",
    sub: "Nhạc chill, thiên nhiên",
    icon: Coffee,
    bg: "from-red-900/40 to-[#0A0A0A]",
    color: "text-red-400",
  },
  {
    label: "Tập trung",
    sub: "Music, lo-fi, study",
    icon: Target,
    bg: "from-indigo-900/40 to-[#0A0A0A]",
    color: "text-indigo-400",
  },
  {
    label: "Năng lượng",
    sub: "EDM, Workout, Sport",
    icon: Zap,
    bg: "from-pink-900/40 to-[#0A0A0A]",
    color: "text-pink-400",
  },
  {
    label: "Công nghệ",
    sub: "Review, Hướng dẫn",
    icon: Tv,
    bg: "from-blue-900/40 to-[#0A0A0A]",
    color: "text-blue-400",
  },
  {
    label: "Sáng tạo",
    sub: "Thiết kế, DIY, Vlog",
    icon: Palette,
    bg: "from-orange-900/40 to-[#0A0A0A]",
    color: "text-orange-400",
  },
  {
    label: "Giải trí",
    sub: "Hài hước, Meme, Fun",
    icon: Laugh,
    bg: "from-purple-900/40 to-[#0A0A0A]",
        ...
        >
          {video.channelAvatarUrl ? (
            <img
              ...
              alt=""
              className="w-8 h-8 rounded-full object-cover ring-2 ring-transparent hover:ring-[#FF5722] transition-all"
            />
          ) : (
...
export default function Explore() {
  ...
  useEffect(() => {
    axios
      .get("/api/videos/explore?sort=views&limit=8")
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/videos/explore?${params.toString()}`)
        ...
        <div className="relative z-10 max-w-[1500px] w-full mx-auto px-6 md:px-10">
          ...
          <div className="flex items-center gap-6">
            <form
              ...
              className="flex items-center bg-[#1A1A1A]/90 backdrop-blur-md rounded-full p-1.5 w-full max-w-xl border border-white/10 shadow-2xl"
            >
              ...
              {inputValue && (
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-white mr-1 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              )}
          ...
          {searchQuery || activeCategory !== "all" ? (
                ...
                <h2 className="text-[19px] font-bold text-white flex items-center gap-2">
                  ...
                  {searchQuery ? (
                    <>
                      ...
                      <span className="text-[#FF5722]">{searchQuery}</span>"
                    </>
                  ) : (
                    <>
                      Danh mục:{" "}
                      <span className="text-[#FF5722]">{activeCategory}</span>
                    </>
                  )}
                </h2>
                <button
                  ...
                  className="text-[13px] font-medium text-gray-400 hover:text-white flex items-center gap-1 transition-colors cursor-pointer"
                >
            ...
            <>
              ...
              <section>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button
                    ...
                    className="text-[13px] font-medium text-gray-400 hover:text-white flex items-center gap-1 transition-colors group cursor-pointer"
                  >
                    Xem tất cả{" "}
              ...
              <section>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-[19px] font-bold text-white flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-[#FF5722]" /> Video mới
                    cập nhật
                  </h2>
                  <button
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 border-b border-white/8 shadow-xl">
            <h3 className="text-[15px] font-bold text-white flex items-center gap-2 mb-5">
              <TrendingUp className="w-4 h-4 text-[#E91E63]" /> Xu hướng tìm
              kiếm
            </h3>
                    ...
                    <Link to={`/c/${channel.handle}`} className="shrink-0">
                      {channel.avatarUrl ? (
                        <img
                          ...
                          alt=""
                          className="w-10 h-10 rounded-full object-cover group-hover:ring-2 ring-transparent ring-[#FF5722] transition-all"
                        />
                      ) : (
          ...
          {dbCategories.slice(0, 6).map((cat, idx) => {
            ...
            return (
              <div
                ...
                className={`shrink-0 w-[220px] p-5 rounded-2xl bg-gradient-to-br ${item.bg} to-[#0A0A0A] border border-white/5 flex items-center gap-4 hover:border-white/20 hover:-translate-y-1 transition-all cursor-pointer group shadow-lg`}
              >
                <div
                  className={`w-12 h-12 shrink-0 rounded-full bg-white/5 flex items-center justify-center ${item.color} group-hover:scale-110 transition-transform`}
                >
