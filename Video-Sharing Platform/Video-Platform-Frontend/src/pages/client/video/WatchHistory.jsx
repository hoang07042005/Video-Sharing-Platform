...
export default function WatchHistory() {
  ...
  const clearAll = async () => {
    if (window.confirm("Xoá tất cả lịch sử xem?")) {
      try {
        await axios.delete("/api/videos/history", {
          headers: { Authorization: "Bearer " + token },
        });
        setAllVideos([]);
  ...
  const grouped = useMemo(
    () => groupByDate(filtered.slice(0, showCount)),
    [filtered, showCount],
  );

  // Stats
  const totalHours = Math.floor(
    allVideos.reduce((s, v) => s + (v.duration || 0), 0) / 3600,
  );
      ...
      <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col items-center justify-center gap-5">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">
            Đăng nhập để xem lịch sử các video bạn đã xem
          </p>
        </div>
        <button
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              Những video bạn đã xem gần đây
            </p>
          </div>
        </div>
          ...
          <div className="sticky top-6 space-y-5">
            ...
            <div className="border-b border-white/8 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[11px] text-gray-500 bg-[#1A1A1A] px-2 py-1 rounded-full">
                  7 ngày qua
                </span>
              </div>
            ...
            <div className="border-b border-white/8 p-5">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <div
                    className={`w-10 h-5 rounded-full transition-colors relative ${paused ? "bg-[#FF5722]" : "bg-[#2A2A2A]"}`}
                  >
                    <div
              ...
              <div className="space-y-1">
                {[
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <item.icon
                        className={`w-5 h-5 ${activeTab === item.key ? "text-[#FF5722]" : ""}`}
                      />
                      {item.label}
                    </span>
                    <span
                      className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${activeTab === item.key ? "bg-[#FF5722]/20 text-[#FF5722]" : "bg-[#1A1A1A] text-gray-500"}`}
                    >
                      {item.count}
                    </span>
                  </button>
