...
export default function VideoDetail() {
  ...
  const [openVideoMenu, setOpenVideoMenu] = useState(null); // videoId | null
  const [showSaveDropdownFor, setShowSaveDropdownFor] = useState(null); // videoId | null
  ...
  // Comment Menu & Report States
  const [openCommentMenu, setOpenCommentMenu] = useState(null); // commentId | null
  ...
  useEffect(() => {
    if (!openVideoMenu && !showSaveDropdownFor) return;
    const handleClickOutside = () => {
      setOpenVideoMenu(null);
      setShowSaveDropdownFor(null);
    };
    document.addEventListener("click", handleClickOutside);
    return () => document.removeEventListener("click", handleClickOutside);
  }, [openVideoMenu, showSaveDropdownFor]);
  ...
  const fetchVideoData = async () => {
    ...
    try {
      ...
      if (
        !data.videoUrl ||
        data.videoUrl.includes("example.com") ||
        data.videoUrl.includes("commondatastorage")
      ) {
      ...
      if (data.resolutions && data.resolutions.length > 0) {
        ...
        const bestRes =
          data.resolutions.find((r) => r.resolution === "1080p") ||
          data.resolutions[0];
      ...
      if (channelId) {
        try {
          ...
          const chVideos = (chRes.data || []).filter(
            (v) => v.id !== id && !v.isShort,
          );
      ...
      if (listId) {
        try {
          const plRes = await axios.get(`/api/playlists/${listId}/videos`, {
            headers,
          });
      ...
      setError(
        `Lỗi: ${err.message} ${err.response ? `(Mã lỗi: ${err.response.status})` : ""}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `/api/channels/${video.channelId}/follow`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      if (err.response?.status === 403) {
        setBanModal({
          message:
            err.response?.data?.message || "Kênh của bạn đã bị đình chỉ.",
        });
      } else {
        const msg =
          err.response?.data?.message ||
          err.response?.data ||
          "Có lỗi xảy ra khi bình luận";
  ...
  const handleCommentReportSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `/api/videos/comments/${reportingCommentId}/report`,
        {
      ...
      alert(
        "Cảm ơn bạn. Báo cáo bình luận của bạn đã được gửi và sẽ được xem xét.",
      );
      ...
      console.error(err);
      alert("Có lỗi xảy ra khi gửi báo cáo bình luận.");
    } finally {
  ...
  const handleReplyLike = async (commentId, replyId, isLike) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(
        `/api/videos/comments/replies/${replyId}/like`,
        { isLike },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
          ...
          if (c.id === commentId && c.replies) {
            return {
              ...
              replies: c.replies.map((r) =>
                r.id === replyId
                  ? { ...r, likesCount: res.data.likesCount }
                  : r,
              ),
            };
          }
  ...
  const renderReplyInput = (targetId, parentId) => {
    ...
    return (
      <div
        className={`flex gap-4 mt-4 w-full ${targetId === parentId ? "ml-14" : ""}`}
      >
        ...
        <div className="flex-1 flex flex-col gap-2 relative z-10">
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="px-3 py-1.5 hover:bg-[#3F3F3F] rounded-full text-white text-sm font-medium transition-colors cursor-pointer"
            >
              Hủy
            </button>
            <button
              onClick={() => handleReply(parentId)}
              className={`px-3 py-1.5 rounded-full text-[#0F0F0F] text-sm font-medium transition-colors ${replyText.trim() ? "bg-[#3EA6FF] hover:bg-[#65B8FF] cursor-pointer" : "bg-[#272727] text-gray-500"}`}
            >
              Phản hồi
            </button>
          </div>
  ...
  const renderReplyTree = (replies, commentId, level = 1) => {
    ...
    return (
      <div className={`relative ${containerMargin} mt-3 z-0`}>
        {/* 12px bridge upward to connect to parent comment across mt-3 gap */}
        <div
          className="absolute left-[-25px] w-[1.5px] bg-[#3A3A3A] z-0"
          style={{ top: "-12px", height: "30px" }}
        />
        ...
        <div className="flex flex-col">
          {replies.map((reply, index) => {
            ...
            return (
              <div
                key={reply.id}
                className={`relative flex flex-col z-10 ${!isLast ? "pb-3" : ""}`}
              >
                {!isLast ? (
                  <>
                    {/* ├ with curve: └ shape into avatar + vertical continues below */}
                    <div className="absolute top-0 left-[-25px] w-5 h-[20px] border-l-[2px] border-b-[1.5px] border-[#3A3A3A] rounded-bl-[10px] z-0" />
                    <div className="absolute top-[10px] left-[-25px] w-[1.5px] h-[calc(100%-10px)] bg-[#3A3A3A] z-0" />
                  </>
                ) : (
                  /* └ curve only: stops at avatar center, line ends here */
                  <div className="absolute top-0 left-[-25px] w-5 h-[20px] border-l-[2px] border-b-[1.5px] border-[#3A3A3A] rounded-bl-[10px] z-0" />
                )}
                ...
                <div className="flex gap-3 relative z-10">
                  <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 mt-1">
                    <img
                      src={
                        reply.avatarUrl ||
                        `https://ui-avatars.com/api/?name=${reply.fullName}`
                      }
                      alt={reply.fullName}
                      className="w-full h-full object-cover"
                    />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isOwner && reply.userId === video.ownerUserId && (
                        <span className="text-[10px] text-[#FF5722] border border-[#FF5722] rounded px-1.5 py-0.5 font-medium">
                          Tác giả
                        </span>
                      )}
                    ...
                    {/* Buttons Action */}
                    <div className="flex items-center gap-4 mt-2 group/reply">
                      <button
                        onClick={() =>
                          handleReplyLike(commentId, reply.id, true)
                        }
                      ...
                      >
                        ...
                        <span className="text-xs">
                          {reply.likesCount > 0
                            ? reply.likesCount.toLocaleString("vi-VN")
                            : ""}
                        </span>
                      </button>
                      <button
                      ...
                      <div className="relative ml-auto">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenCommentMenu(
                              openCommentMenu === reply.id ? null : reply.id,
                            );
                          }}
                          className="w-7 h-7 flex items-center justify-center rounded-full text-gray-500 hover:text-white hover:bg-white/10 transition-all opacity-0 group-hover/reply:opacity-100 cursor-pointer"
                        >
                          <MoreHorizontal className="w-4 h-4" />
                        </button>
                        {openCommentMenu === reply.id && (
                          <div
                            className="absolute right-0 top-full mt-1 w-44 bg-[#212121] border border-white/10 rounded-xl shadow-2xl py-1 z-50"
                          ...
                          >
                            <button
                              onClick={() => {
                                ...
                                if (
                                  !requireAuth("Vui lòng đăng nhập để báo cáo!")
                                )
                                  return;
                                setReportingCommentId(reply.id);
                                setShowCommentReportModal(true);
                              }}
                              className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 text-red-400 text-sm transition-colors cursor-pointer"
                            >
    ...
    <div className="flex-1 bg-[#0F0F0F] overflow-y-auto custom-scrollbar flex justify-center">
      ...
      <div className="flex flex-col lg:flex-row px-2 md:px-4 py-6 gap-3 xl:gap-4 w-full max-w-[1800px]">
        ...
        <div className="flex-1 max-w-[1280px] lg:w-[70%] xl:w-[75%]">
            ...
            {video.isMembersOnly ? (
              <div className="w-full h-full flex flex-col items-center justify-center bg-[#1A1A1A] text-center p-8">
                ...
                <h2 className="text-2xl font-bold text-white mb-2">
                  Video dành riêng cho hội viên
                </h2>
                <p className="text-gray-400 mb-6 max-w-md">
                  Kênh này yêu cầu bạn phải là hội viên mới có thể xem video.
                  Hãy tham gia ngay để ủng hộ kênh và tận hưởng các đặc quyền!
                </p>
                <button
                  onClick={() =>
                    navigate(`/c/${video.channel.handle}/membership`)
                  }
              ...
              <div className="absolute bottom-16 right-4 z-50 bg-black/80 backdrop-blur-md rounded-xl p-4 w-[340px] border border-white/10 shadow-2xl animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    <h4 className="text-white text-sm font-semibold line-clamp-2 leading-snug group-hover:text-blue-400 transition-colors">
                      {nextVideo.title}
                    </h4>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <Play className="w-4 h-4 fill-white" /> Phát ngay (
                    {upNextCountdown})
                  </button>
                </div>
              </div>
                  ...
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden bg-[#2A2A2A]">
                    <img
                      src={
                        ...
                        "https://ui-avatars.com/api/?name=" + video.channelName
                      }
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-full font-bold text-[13px] transition-colors ${
                        isSubscribed
                          ...
                          : "bg-[#FF5722] text-white hover:brightness-110 shadow-lg shadow-[#FF5722]/20"
                      }`}
                    >
              ...
              <div className="flex items-center gap-2 flex-wrap pb-2 md:pb-0">
                <div className="flex items-center bg-[#272727] hover:bg-[#3F3F3F] transition-colors rounded-full overflow-hidden">
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 text-white border-r border-white/20 hover:bg-white/10 cursor-pointer ${isLiked ? "text-blue-500" : ""}`}
                  >
                    <ThumbsUp
                      className={`w-5 h-5 ${isLiked ? "fill-blue-500" : ""}`}
                    />
                    <span className="text-sm font-medium">
                      {likesCount ? likesCount.toLocaleString("vi-VN") : "0"}
                    </span>
                  </button>
                  <button
                    onClick={() => handleLike(false)}
                    className={`px-4 py-2 text-white hover:bg-white/10 cursor-pointer ${isDisliked ? "text-blue-500" : ""}`}
                  >
                    <ThumbsDown
                      className={`w-5 h-5 ${isDisliked ? "fill-blue-500" : ""}`}
                    />
                  </button>
                </div>
                ...
                <div className="relative">
                  <button
                    onClick={() => {
                      if (!requireAuth("Vui lòng đăng nhập để lưu video!"))
                        return;
                      ...
                      {publicSettings?.allowDownloads !== false && (
                        <button
                          onClick={() => {
                            ...
                            if (
                              !requireAuth("Vui lòng đăng nhập để tải video!")
                            )
                              return;

                            if (tier < 1) {
                              toast.error(
                                "Vui lòng nâng cấp gói PLUS trở lên để tải video!",
                              );
                              navigate("/premium");
                              return;
                            }
                        ...
                        >
                          <Download
                            className={`w-5 h-5 ${tier < 1 ? "text-gray-500" : ""}`}
                          />
                          <span className="text-sm font-medium">
                            Tải xuống {tier < 1 && "(Cần PLUS)"}
                          </span>
                        </button>
                      )}
                      <button
                        onClick={() => {
                          ...
                          if (
                            !requireAuth(
                              "Vui lòng đăng nhập để báo cáo vi phạm!",
                            )
                          )
                            return;
          ...
          {/* Description Box */}
          <div
            className="mt-6 bg-[#272727] hover:bg-[#3F3F3F] transition-colors rounded-xl p-4 cursor-pointer"
            onClick={() => setShowFullDescription(!showFullDescription)}
          >
            <div className="text-sm text-white font-medium mb-2">
              <span>
                {video.viewsCount
                  ? video.viewsCount.toLocaleString("vi-VN")
                  : "0"}{" "}
                lượt xem
              </span>
            ...
            <div
              className={`text-sm text-white/90 whitespace-pre-wrap ${!showFullDescription ? "line-clamp-2" : ""}`}
            >
              {video.description ||
                "Diving deep into the neon-lit streets. Showcasing the absolute max settings on the new RTX 5090. Make sure to hit that like button!"}
            </div>
            {!showFullDescription && (
            ...
            {showFullDescription && (
              <button
                className="text-sm font-medium text-white mt-1 hover:underline cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
                ...
                {showSortDropdown && (
                  <div className="absolute top-full left-0 mt-2 w-72 bg-[#272727] rounded-xl shadow-xl py-2 z-50">
                    ...
                    >
                      ...
                      <div className="text-gray-400 text-xs mt-1">
                        Hiện bình luận gần đây, bao gồm cả những mục có thể là
                        bình luận rác
                      </div>
                    </button>
                  </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-end gap-2 mt-3">
                  <button
                    ...
                    className="px-4 py-2 text-white hover:bg-[#272727] rounded-full text-sm font-medium transition-colors cursor-pointer"
                  >
                    Hủy
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium ${commentText.trim() ? "bg-blue-500 text-white cursor-pointer hover:bg-blue-600" : "bg-[#272727] text-gray-400 cursor-not-allowed"}`}
                  >
                    Bình luận
                  </button>
                </div>
            ...
            <div className="flex flex-col gap-6">
              {sortedComments.map((comment) => (
                <div key={comment.id} className="flex flex-col mb-6 relative">
                  {/* Parent Comment */}
                  <div className="flex gap-4 relative z-10 group/comment">
                    <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 mt-1">
                      <img
                        src={
                          comment.avatarUrl ||
                          `https://ui-avatars.com/api/?name=${comment.fullName}`
                        }
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span
                          className={`font-bold text-[13px] ${comment.isPremium ? "text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-pink-500" : "text-white"}`}
                        >
                          {comment.fullName}
                        </span>
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        >
                          ...
                          <span className="text-xs">
                            {comment.likesCount > 0
                              ? comment.likesCount.toLocaleString("vi-VN")
                              : ""}
                          </span>
                        </button>
                        <button
                        ...
                        <div className="relative ml-auto">
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center rounded-full text-gray-500 hover:text-white hover:bg-white/10 transition-all opacity-0 group-hover/comment:opacity-100 cursor-pointer"
                          >
                            <MoreHorizontal className="w-4 h-4" />
                          </button>
                          {openCommentMenu === comment.id && (
        ...
        <div className="lg:w-[30%] xl:w-[25%]">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <div
                  className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${autoplay ? "left-[22px]" : "left-0.5"}`}
                ></div>
              </button>
            </div>
          </div>
          <div className="border-t border-white/10 pt-5">
            ...
            {playlistData ? (
              <div
                className="bg-[#141418] rounded-xl border border-white/10 mb-6 flex flex-col"
                style={{ maxHeight: "540px" }}
              >
                {/* Sticky Header */}
                <div className="p-4 bg-[#141418] rounded-t-xl shrink-0">
                  <h3 className="text-white font-bold text-base mb-1 line-clamp-1">
                    {playlistData.title}
                  </h3>
                  <p className="text-gray-400 text-xs">
                    {video.channelName} ·{" "}
                    {playlistData.videos?.findIndex((v) => v.id === id) + 1} /{" "}
                    {playlistData.videos?.length} video
                  </p>
                </div>
                {/* Scrollable List */}
                <div className="flex flex-col gap-0.5 p-2 overflow-y-auto flex-1 min-h-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-[#0F0F0F] [&::-webkit-scrollbar-thumb]:bg-[#374151] [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-[#4b5563] [scrollbar-width:thin] [scrollbar-color:#374151_#0F0F0F]">
                  {playlistData.videos?.map((pv, idx) => (
                    <Link
                      key={pv.id}
                      to={`/watch/${pv.id}?list=${playlistData.id}`}
                      className={`flex items-center gap-3 p-2 rounded-lg transition-colors group ${pv.id === id ? "bg-white/10" : "hover:bg-white/5"}`}
                    >
                      {/* Index / Playing indicator */}
                      <div className="w-5 flex items-center justify-center shrink-0">
                        {pv.id === id ? (
                          <Play className="w-3.5 h-3.5 text-[#FF5722] fill-[#FF5722]" />
                        ) : (
                        ...
                        {pv.id === id && (
                          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                            <div className="flex gap-0.5 items-end h-3">
                              <span
                                className="w-[3px] bg-[#FF5722] rounded-full animate-[eq1_0.8s_ease-in-out_infinite]"
                                style={{ height: "40%" }}
                              ></span>
                              <span
                                className="w-[3px] bg-[#FF5722] rounded-full animate-[eq2_0.8s_ease-in-out_infinite_0.15s]"
                                style={{ height: "80%" }}
                              ></span>
                              <span
                                className="w-[3px] bg-[#FF5722] rounded-full animate-[eq1_0.8s_ease-in-out_infinite_0.3s]"
                                style={{ height: "60%" }}
                              ></span>
                            </div>
                          </div>
                      ...
                      <div className="flex flex-col min-w-0 flex-1">
                        <span
                          className={`text-xs font-semibold line-clamp-2 leading-tight mb-0.5 ${pv.id === id ? "text-[#FF5722]" : "text-white group-hover:text-white/90"}`}
                        >
                          {pv.title}
                        </span>
                        <div className="flex items-center gap-1 min-w-0">
                          ...
                          {pv.channelIsVerified && (
                            <CheckCircle className="w-2.5 h-2.5 text-white fill-green-500 shrink-0" />
                          )}
                        </div>
                        <div className="flex items-center gap-1 mt-0.5 text-gray-500 text-[10px]">
                          {pv.viewsCount > 0 && (
                            <span>{formatViews(pv.viewsCount)} lượt xem</span>
                          )}
                          {pv.viewsCount > 0 && pv.createdAt && <span>·</span>}
                          {pv.createdAt && (
              ...
              sameChannelVideos.length > 0 && (
                <div className="flex flex-col gap-3 mb-6">
                  {sameChannelVideos.map((recVideo) => (
                    <div
                      key={recVideo.id}
                      className="relative flex gap-2 group/item"
                    >
                      <Link
                        to={
                          recVideo.isShort
                            ? `/shorts?id=${recVideo.id}`
                            : `/watch/${recVideo.id}`
                        }
                        className="flex gap-2 flex-1 min-w-0 group"
                      >
                        ...
                        <div className="flex flex-col flex-1 py-0.5 min-w-0 pr-6">
                          <h5 className="text-white text-[13px] font-semibold line-clamp-2 leading-tight group-hover:text-[#FF5722] transition-colors">
                            {recVideo.title}
                          </h5>
                          <div className="flex items-center gap-1 min-w-0">
                            ...
                            {recVideo.channelIsVerified && (
                              <CheckCircle className="w-3 h-3 text-white fill-green-500 shrink-0" />
                            )}
                          </div>
                          <div className="text-gray-400 text-[11px] flex items-center gap-1 mt-0.5 flex-wrap">
                            <span>
                              {formatViews(recVideo.viewsCount)} lượt xem
                            </span>
                            <span>•</span>
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          setOpenVideoMenu(
                            openVideoMenu === recVideo.id ? null : recVideo.id,
                          );
                        }}
                        className="absolute top-1 right-0 w-7 h-7 flex items-center justify-center rounded-full bg-[#1A1A1A]/90 hover:bg-[#3F3F3F] text-white opacity-0 group-hover/item:opacity-100 transition-opacity duration-150 cursor-pointer z-10"
                      >
                        <MoreHorizontal className="w-4 h-4" />
                      </button>
                      {/* Dropdown */}
                      {openVideoMenu === recVideo.id && (
                        <div
                          className="absolute right-0 top-8 w-52 bg-[#212121] border border-white/10 rounded-xl shadow-2xl py-1 z-50"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            onClick={() => {
                              setShowSaveDropdownFor(recVideo.id);
                              setOpenVideoMenu(null);
                            }}
                          ...
                          <button
                            onClick={() => {
                              requireAuth("Vui lòng đăng nhập để lưu video!") &&
                                setOpenVideoMenu(null);
                            }}
                            className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 text-white text-sm transition-colors cursor-pointer"
                          >
                          ...
                          <button
                            onClick={() => {
                              ...
                              addDownload({
                                ...
                                viewsCount: recVideo.viewsCount,
                                channelName: recVideo.channelName,
                                channelHandle: recVideo.channelHandle,
                              });
                            }}
                            className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 text-white text-sm transition-colors cursor-pointer"
                          >
                      ...
                      {showSaveDropdownFor === recVideo.id && (
                        <div
                        ...
                        >
                          <SaveToPlaylistDropdown
                            videoId={recVideo.id}
                            onClose={() => setShowSaveDropdownFor(null)}
                          />
                        </div>
                      )}
                    </div>
                      ...
                      >
                        <div className="relative w-full aspect-[4/6] rounded-xl overflow-hidden bg-[#1A1A1A]">
                          ...
                          <div className="absolute bottom-2 right-2 bg-black/80 text-white text-[11px] px-1.5 py-0.5 rounded font-medium">
                            ...
                            {(short.duration % 60).toString().padStart(2, "0")}
                          </div>
                        </div>
                ...
                .map((recVideo) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col flex-1 py-1 min-w-0 pr-6">
                        <h5 className="text-white text-[13px] font-semibold line-clamp-3 leading-tight group-hover:text-[#FF5722] transition-colors">
                          {recVideo.title}
                        </h5>
                        <div className="flex items-center gap-1 min-w-0">
                          <span className="text-gray-400 text-xs mt-1 truncate">
                    ...
                    {openVideoMenu === recVideo.id && (
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            setOpenVideoMenu(null);
                            if (!requireAuth("Vui lòng đăng nhập để báo cáo!"))
                              return;
                            setShowReportModal(true);
                          }}
                          className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 text-red-400 text-sm transition-colors cursor-pointer"
            ...
            <div className="p-4 flex flex-col gap-4">
              <div className="space-y-3">
                ...
                {[
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${reportReason === reason ? "border-[#FF5722]" : "border-gray-500 group-hover:border-gray-400"}`}
                    >
                      {reportReason === reason && (
                    ...
                    <span
                      className={`text-sm ${reportReason === reason ? "text-white" : "text-gray-300 group-hover:text-white"}`}
                    >
                      {reason}
                    </span>
                  </label>
          ...
          >
            ...
            <div className="p-4 flex flex-col gap-4">
              <div className="space-y-3">
                ...
                {[
                  "Spam hoặc quảng cáo",
                  "Ngôn từ thù ghét hoặc xúc phạm",
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${commentReportReason === reason ? "border-[#FF5722]" : "border-gray-500 group-hover:border-gray-400"}`}
                    >
                    ...
                    <span
                      className={`text-sm ${commentReportReason === reason ? "text-white" : "text-gray-300 group-hover:text-white"}`}
                    >
                      {reason}
                    </span>
                  </label>
              ...
              <div className="mt-2">
                ...
                <textarea
                  ...
                  placeholder="Mô tả thêm về vi phạm của bình luận này..."
                  className="w-full bg-[#151515] border border-white/10 rounded-xl p-3 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-[#FF5722]/50 resize-none h-20"
                />
              </div>
            </div>
