      ...
      <div className="flex items-start justify-between gap-2 px-0.5">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-gray-500 text-[11px]">
            Cập nhật {timeAgo(playlist.createdAt)} •{" "}
            {playlist.visibility === "Private" ? "Riêng tư" : "Công khai"}
          </p>
        </div>
...
function PlaylistListItem({ playlist, onOpen, onDelete, onEdit }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h3 className="text-white font-semibold text-sm group-hover:text-[#FF5722] transition-colors line-clamp-1 mb-1">
          {playlist.title}
        </h3>
...
function ActivityItem({ playlist, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-white text-[12px] font-semibold line-clamp-1 group-hover:text-[#FF5722] transition-colors">
          {playlist.title}
        </p>
        <p className="text-gray-500 text-[10px] mt-0.5">
          Thêm {playlist.videoCount ?? 0} video mới
        </p>
...
export default function Playlists() {
  ...
  const openPlaylist = (playlist) => {
    ...
    axios
      .get(`/api/playlists/${playlist.id}/videos`, {
        headers: { Authorization: "Bearer " + token },
      })
      .then((r) => {
        setSelected((prev) => ({
  ...
  const privateCount = playlists.filter(
      ...
      <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col items-center justify-center gap-5">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">
            Đăng nhập để xem và quản lý danh sách phát
          </p>
        </div>
        <button
    ...
    <div className="min-h-screen bg-[#0F0F0F] text-white font-sans pb-20">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 mt-8 mb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center gap-4 mb-6">
            <button
              ...
              className="p-2 bg-[#1A1A1A] hover:bg-[#2A2A2A] rounded-full text-white transition-colors cursor-pointer"
            >
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs mt-1">
                {selected.totalVideos || selected.videoCount} video
              </p>
            </div>
          </div>
            ...
            {detailLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-gray-500 text-base">
                  Danh sách này chưa có video nào
                </p>
              </div>
            ) : (
      ...
      {showEdit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-black/60 backdrop-blur-sm"
            onClick={() => !isEditing && setShowEdit(false)}
          />
          <div className="relative z-50 w-full max-w-md bg-[#181818] rounded-2xl shadow-2xl overflow-hidden border border-white/10 p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-white">
      ...
      {!selected && (
        <>
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg,#1a0533 0%,#0d1a3a 50%,#1a0533 100%)",
            }}
          >
            {/* Decorative blobs */}
            <div
            ...
            />
            <div
              className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full opacity-20 blur-3xl"
              style={{
                ...
                <div className="flex items-center gap-4 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 text-sm mt-0.5">
                      Quản lý và sắp xếp các danh sách phát của bạn
                    </p>
                  </div>
                </div>
                ...
                <div className="text-center">
                  <p className="text-white font-semibold mb-1">
                    {activeTab === "all"
                      ...
                      : `Không có danh sách ${activeTab === "public" ? "công khai" : "riêng tư"} nào`}
                  </p>
                  <p className="text-gray-500 text-sm">
                    Tạo danh sách phát đầu tiên của bạn ngay!
                  </p>
                </div>
              </div>
            ) : viewMode === "grid" ? (
              ...
              <div className="bg-[#111] border border-white/5 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-[15px] font-bold text-white flex items-center gap-2">
                    <Activity className="w-4 h-4 text-[#FF5722]" /> Hoạt động
                    gần đây
                  </h2>
