...
export default function NotificationsPage() {
  ...
  const getFallbackTitle = (type) => {
    switch(type?.toLowerCase()) {
      ...
      case 'subscribe': return 'Người đăng ký mới';
      case 'follow': return 'Người theo dõi mới';
      case 'donation': return 'Ủng hộ mới';
      case 'stream': return 'Cập nhật Stream';
      case 'communitypost': return 'Bài viết cộng đồng';
      case 'feedbackreply': return 'Phản hồi hỗ trợ';
  ...
  const getNotificationIcon = (type, title) => {
    ...
    if (t === 'subscribe' || t === 'follow') return <UserPlus className="w-5 h-5 text-[#3b82f6]" />;
    if (t === 'comment') return <MessageSquare className="w-5 h-5 text-[#f97316]" />;
    if (t === 'donation') return <DollarSign className="w-5 h-5 text-[#eab308]" />;
    if (t === 'stream') return <PlayCircle className="w-5 h-5 text-[#a855f7]" />;
    if (t === 'communitypost') return <MessageSquare className="w-5 h-5 text-[#a855f7]" />;
    if (t === 'feedbackreply') return <CheckCircle2 className="w-5 h-5 text-[#14b8a6]" />;
    if (t === 'system') {
      if (ttl.includes('rút tiền')) return <Wallet className="w-5 h-5 text-[#ef4444]" />;
      if (ttl.includes('hội viên') || ttl.includes('vip')) return <Crown className="w-5 h-5 text-[#eab308]" />;
      if (ttl.includes('bảo mật') || ttl.includes('đăng nhập')) return <Shield className="w-5 h-5 text-gray-300" />;
  ...
  const getNotificationColor = (type, title) => {
    const t = type?.toLowerCase();
    const ttl = title?.toLowerCase() || '';
    if (t === 'subscribe' || t === 'follow') return "bg-[#3b82f6]/20 border-transparent";
    if (t === 'comment') return "bg-[#f97316]/20 border-transparent";
    if (t === 'donation') return "bg-[#eab308]/20 border-transparent";
    if (t === 'stream' || t === 'communitypost') return "bg-[#a855f7]/20 border-transparent";
    if (t === 'feedbackreply') return "bg-[#14b8a6]/20 border-transparent";
    if (t === 'system') {
      if (ttl.includes('rút tiền')) return "bg-[#ef4444]/20 border-transparent";
      if (ttl.includes('hội viên') || ttl.includes('vip')) return "bg-[#eab308]/20 border-transparent";
      if (ttl.includes('bảo mật') || ttl.includes('đăng nhập')) return "bg-gray-600/40 border-transparent";
  ...
  const getNotificationGlowColor = (type, title) => {
    ...
    if (t === 'system') {
      ...
      if (ttl.includes('hội viên') || ttl.includes('vip')) return "bg-[#eab308]";
      if (ttl.includes('bảo mật') || ttl.includes('đăng nhập')) return "bg-gray-500";
      return "bg-[#f97316]";
    }
    return "bg-gray-500";
  };

  const getNotificationBorderColor = (type, title) => getNotificationGlowColor(type, title).replace('bg-', 'border-');
  ...
  const getNotificationTheme = (type, title) => {
    ...
    } else if (t === 'system' && (ttl.includes('hội viên') || ttl.includes('vip'))) {
      hex = '#eab308';
      bgGradient = 'from-[#111111] via-[#1c1a0f] to-[#2a2508]';
      text = 'text-[#eab308]';
      activeBg = 'bg-[#eab308]/10';
      activeBorder = 'border-l-[#eab308]';
    } else if (t === 'system' && (ttl.includes('bảo mật') || ttl.includes('đăng nhập'))) {
      hex = '#9ca3af';
  ...
  const parseSystemMessage = (message) => {
    ...
    lines.forEach(line => {
      ...
      if (trimmed.startsWith('- ')) {
        ...
        if (parts.length >= 2) {
          details.push({ key: parts[0].trim(), value: parts.slice(1).join(':').trim() });
        }
      } else {
  ...
  const filteredNotifications = notifications.filter(notif => {
    ...
    if (searchQuery) {
      ...
      return titleLower.includes(searchLower) || messageLower.includes(searchLower);
    }
    return true;
  });
    ...
    <div className="pt-20 px-4 md:px-8 max-w-[1400px] mx-auto min-h-screen flex flex-col pb-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 shrink-0 mt-6">
        <div>
          ...
          <p className="text-gray-400">Xem và quản lý tất cả các hoạt động liên quan đến bạn</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1">
        ...
        <div className="flex border-b border-white/5">
          ...
          >
            Tất cả
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-bold ${activeTab === 'all' ? 'bg-[#ef4444] text-white' : 'bg-white/10 text-gray-400'}`}>
          ...
          >
            Chưa đọc
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-bold ${activeTab === 'unread' ? 'bg-[#ef4444] text-white' : 'bg-white/10 text-gray-400'}`}>
        ...
        <div className="flex flex-col flex-1">
          {loading ? (
            ...
            filteredNotifications.map(notif => {
              ...
              >
                <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center shrink-0 border ${getNotificationColor(notif.type, notif.title)}`}>
                  {getNotificationIcon(notif.type, notif.title)}
                </div>
                
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4">
                    <p className={`text-[15px] leading-relaxed line-clamp-1 ${!notif.isRead ? 'text-white font-medium' : 'text-gray-300'}`}>
                      {notif.title}
                    </p>
            ...
            <div className="p-16 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Không có thông báo nào</h3>
              <p className="text-gray-400">Bạn hiện không có thông báo nào trong danh mục này.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {selectedNotif ? (() => {
        const selectedTheme = getNotificationTheme(selectedNotif.type, selectedNotif.title);
        return (
      <div className={`hidden lg:flex flex-1 flex-col bg-gradient-to-b ${selectedTheme.bgGradient} border border-white/5 rounded-2xl overflow-hidden shadow-xl relative`}>
          <div className="flex flex-col h-full animate-in fade-in duration-300">
            {/* Ambient Glow */}
            <div className="absolute inset-0 pointer-events-none rounded-2xl overflow-hidden z-0">
              <div className={`absolute -top-20 -right-20 w-[500px] h-[500px] rounded-full blur-[120px] opacity-15 ${getNotificationGlowColor(selectedNotif.type, selectedNotif.title)}`} />
              <div className={`absolute -top-20 -left-20 w-[400px] h-[400px] rounded-full blur-[100px] opacity-10 ${getNotificationGlowColor(selectedNotif.type, selectedNotif.title)}`} />
            </div>
            ...
            <div className="p-10 flex-1 relative z-10">
              <div className="flex flex-col items-center text-center mb-10 mt-2 relative">
                ...
                <div className={`w-32 h-32 rounded-full flex items-center justify-center shrink-0 border mb-8 ${selectedNotif.type?.toLowerCase() === 'system' && selectedNotif.title?.toLowerCase().includes('rút tiền') ? 'bg-[#2a1111] border-[#ef4444]' : getNotificationColor(selectedNotif.type, selectedNotif.title)} relative group mt-8`}>
                  <div className={`absolute inset-0 rounded-full opacity-0 group-hover:opacity-20 transition-opacity duration-500 blur-xl ${getNotificationGlowColor(selectedNotif.type, selectedNotif.title)}`} />
                  <div className="scale-[2.5] relative z-10">
                    {selectedNotif.type?.toLowerCase() === 'system' && selectedNotif.title?.toLowerCase().includes('rút tiền') ? (
                      <Check className="w-6 h-6 text-white font-bold stroke-[3px]" />
                    ) : (
                      getNotificationIcon(selectedNotif.type, selectedNotif.title)
                    )}
                  </div>
                  {/* Sparkles */}
                  <div className={`absolute -top-6 left-0 ${selectedTheme.text} opacity-50 text-xl animate-pulse`}>✦</div>
                  <div className={`absolute -top-2 -right-8 ${selectedTheme.text} opacity-50 text-2xl animate-pulse delay-150`}>✦</div>
                  <div className={`absolute bottom-4 -right-4 ${selectedTheme.text} opacity-50 text-sm animate-pulse delay-300`}>✦</div>
                  <div className={`absolute top-1/2 -left-8 ${selectedTheme.text} opacity-30 text-sm animate-pulse delay-75`}>✦</div>
                </div>
                
                <h2 className="text-3xl font-extrabold text-white mt-6 mb-3 px-8 drop-shadow-lg text-center leading-tight max-w-[90%]">
                  {selectedNotif.title || getFallbackTitle(selectedNotif.type)}
                </h2>
                {selectedNotif.type?.toLowerCase() === 'system' && selectedNotif.title?.toLowerCase().includes('rút tiền') && (
              ...
              {selectedNotif.type?.toLowerCase() === 'system' && selectedNotif.title?.toLowerCase().includes('rút tiền') ? (() => {
                const parsed = parseSystemMessage(selectedNotif.message);
                return (
                  <div className="max-w-2xl mx-auto w-full">
                    <div className="bg-black/5 backdrop-blur-2xl border border-white/5 rounded-[10px] p-6 mb-6">
                      <h3 className="text-white font-semibold mb-6 text-[15px]">Chi tiết giao dịch</h3>
                      <div className="flex flex-col gap-4">
                        {parsed.details.map((detail, idx) => {
                          let Icon = Target;
                          if (detail.key.toLowerCase().includes('mã')) Icon = Calendar;
                          if (detail.key.toLowerCase().includes('tiền')) Icon = UserPlus;
                          if (detail.key.toLowerCase().includes('phương')) Icon = Zap;
                          
                          let valueColor = "text-white";
                          if (detail.key.toLowerCase().includes('mã')) valueColor = "text-[#ef4444]";
                          if (detail.key.toLowerCase().includes('tiền')) valueColor = "text-[#eab308]";
                          
                          return (
                            <div key={idx} className="flex items-center justify-between pb-4 border-b border-white/5 last:border-b-0 last:pb-0">
                              <div className="flex items-center gap-3 text-white">
                                <Icon className="w-5 h-5 text-white" />
                                <span className="text-[15px]">{detail.key}</span>
                              </div>
                              {detail.key.toLowerCase().includes('trạng thái') ? (
                                <span className="text-[13px] text-[#22c55e] bg-green-900/30 px-3 py-1 rounded-md font-semibold">
                                  {detail.value}
                                </span>
                              ) : (
                                <span className={`text-[15px] font-semibold ${valueColor}`}>{detail.value}</span>
                              )}
                            </div>
                          );
                        })}
                    ...
                    <div className="bg-black/5 backdrop-blur-2xl border border-white/5 rounded-[10px] p-6 mb-[100px] relative overflow-hidden">
                      <Quote className={`w-6 h-6 ${selectedTheme.text} mb-4 opacity-70`} />
                      ...
                      <div className="flex flex-col gap-1">
                        <p className="text-sm text-white italic">Trân trọng,</p>
                        <p className="text-[15px] font-semibold text-white">Đội ngũ Quản trị Video Sharing Platform</p>
                      </div>
                      <Send className={`absolute bottom-6 right-6 w-8 h-8 ${selectedTheme.text} opacity-20 -rotate-12`} />
                    </div>
                  </div>
                );
              })() : (
                <div className={`relative bg-black/20 backdrop-blur-2xl p-8 rounded-[10px] border border-white/10 mb-10 overflow-hidden max-w-2xl mx-auto w-full`}>
        ...
        <div className="hidden lg:flex flex-1 flex-col bg-[#111111] border border-white/5 rounded-2xl overflow-hidden shadow-xl relative">
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <h3 className="text-xl font-bold text-white mb-2">Chưa chọn thông báo nào</h3>
            <p className="text-gray-400 max-w-sm">
              Vui lòng chọn một thông báo từ danh sách bên trái để xem chi tiết
            </p>
          </div>
        </div>
      )}
          ...
          <div className="bg-[#151515] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 border ${getNotificationColor(selectedNotif.type)}`}>
                  {selectedNotif.imageUrl ? (
                    <img src={selectedNotif.imageUrl} alt="" className="w-full h-full rounded-full object-cover" />
                  ) : (
