...
import { 
  ...
  User, PlayCircle, DollarSign, Wallet, ShieldAlert, Settings,
  Mail, MessageSquare, Phone, CheckCircle2, CloudUpload, Loader2, Users, Send
} from 'lucide-react';
import { toast } from 'react-toastify';
import axios from 'axios';

const categories = [
  { id: 'account', icon: User, title: 'Tài khoản & Bảo mật', subtitle: 'Đăng nhập, bảo mật, xác minh...', color: 'bg-[#9333ea]' },
  { id: 'video', icon: PlayCircle, title: 'Video & Nội dung', subtitle: 'Tải lên, xử lý, định dạng...', color: 'bg-[#ef4444]' },
  { id: 'payment', icon: DollarSign, title: 'Thanh toán & Nạp tiền', subtitle: 'Nạp xu, thanh toán, hóa đơn...', color: 'bg-[#22c55e]' },
  { id: 'revenue', icon: Wallet, title: 'Rút tiền & Doanh thu', subtitle: 'Rút tiền, doanh thu, thuế...', color: 'bg-[#f59e0b]' },
  { id: 'copyright', icon: ShieldAlert, title: 'Bản quyền & Vi phạm', subtitle: 'Báo cáo, khiếu nại, vi phạm...', color: 'bg-[#3b82f6]' },
  { id: 'technical', icon: Settings, title: 'Lỗi kỹ thuật', subtitle: 'Lỗi hệ thống, không tải được...', color: 'bg-[#8b5cf6]' },
];

const faqs = [
  {
    question: "Làm thế nào để đăng ký gói Hội viên VIP?",
    answer: "Truy cập vào trang cá nhân của kênh bạn yêu thích, bấm vào nút 'Tham gia' (Join) và chọn gói Hội viên phù hợp. Bạn có thể thanh toán qua ví MoMo hoặc thẻ ngân hàng."
  },
  {
    question: "Tại sao tôi không xem được video ở độ phân giải 4K?",
    answer: "Chất lượng 4K yêu cầu kết nối internet ổn định (khuyến nghị trên 20Mbps) và thiết bị hỗ trợ giải mã phần cứng. Vui lòng kiểm tra lại đường truyền và cài đặt hiển thị của bạn."
  },
  {
    question: "Cách báo cáo một video có nội dung xấu?",
    answer: "Bên dưới mỗi trình phát video, hãy bấm vào nút 3 chấm (...) -> chọn 'Báo cáo vi phạm'. Chọn lý do chính xác để đội ngũ kiểm duyệt xử lý nhanh chóng."
  },
  {
    question: "Làm sao để xóa lịch sử xem video?",
    answer: "Bạn hãy vào menu 'Lịch sử' (Watch History) ở thanh điều hướng bên trái, sau đó bấm nút 'Xóa toàn bộ lịch sử xem' ở cột bên phải."
  },
  {
    question: "Thời gian xử lý yêu cầu rút tiền là bao lâu?",
    answer: "Các yêu cầu rút tiền thường được xử lý trong khoảng thời gian từ ngày 10 đến ngày 15 hàng tháng."
  }
];
...
export default function Help() {
  ...
  useEffect(() => {
    axios.get('/api/admin/settings/public')
      .then(res => {
        if (res.data) {
          setSiteConfig(prev => ({
            ...prev,
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ type: 'technical', title: '', content: '', attachmentUrl: '' });
    } catch (err) {
    ...
    <div className="flex-1 bg-[#09090b] min-h-screen text-gray-200 font-sans pb-20">
        ...
        <div className="max-w-[1200px] mx-auto px-6 lg:px-8 relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          ...
          <div className="w-full md:w-[60%] z-10">
            ...
            <p className="text-gray-400 text-[15px] mb-8 max-w-xl">
              Bạn gặp vấn đề hoặc cần hỗ trợ? Hãy tìm câu trả lời nhanh chóng hoặc gửi yêu cầu cho chúng tôi.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-6 text-sm">
              ...
              {['rút tiền', 'lỗi video', 'bản quyền', 'tài khoản', 'VIP'].map(tag => (
                <button 
                  key={tag} 
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-8 mt-12">
        ...
        <div className="mb-14">
          ...
          <p className="text-sm text-gray-500 mb-6">Chọn danh mục phù hợp để tìm kiếm câu trả lời nhanh hơn</p>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categories.map((cat, idx) => (
              <div key={idx} className="bg-[#141417] border border-white/5 rounded-2xl p-5 flex items-center gap-4 hover:bg-white/[0.04] transition-colors cursor-pointer group">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${cat.color} shadow-lg shadow-black/50`}>
                  <cat.icon className="w-6 h-6 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-200 text-[15px] truncate">{cat.title}</h3>
                  <p className="text-[13px] text-gray-500 truncate mt-0.5">{cat.subtitle}</p>
                </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 mb-20">
          ...
          <div className="w-full lg:w-[65%] flex flex-col gap-10">
            ...
            <div>
              <div className="flex items-end justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold text-white mb-1">Câu hỏi thường gặp</h2>
                  <p className="text-sm text-gray-500">Những câu hỏi phổ biến nhất từ cộng đồng</p>
                </div>
              ...
              <div className="space-y-2">
                {filteredFaqs.length > 0 ? (
                  filteredFaqs.map((faq, idx) => (
                    <div key={idx} className="bg-[#141417] border border-white/5 rounded-xl overflow-hidden transition-all duration-300">
                      <button 
                        onClick={() => setOpenFaq(openFaq === idx ? -1 : idx)}
                        className="w-full flex items-center justify-between p-4 text-left cursor-pointer hover:bg-white/[0.02]"
                      >
                        <span className={`font-semibold text-[15px] pr-8 ${openFaq === idx ? 'text-white' : 'text-gray-300'}`}>
                          {faq.question}
                        </span>
                        {openFaq === idx ? <ChevronUp className="w-4 h-4 text-gray-500 shrink-0" /> : <ChevronDown className="w-4 h-4 text-gray-500 shrink-0" />}
                      </button>
                      
                      <div className={`px-4 overflow-hidden transition-all duration-300 ease-in-out ${openFaq === idx ? 'max-h-96 pb-4 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="bg-[#141417] border border-white/5 rounded-xl p-8 text-center">
                    <p className="text-gray-400">Không tìm thấy câu hỏi phù hợp với "{searchQuery}"</p>
                    <p className="text-sm text-gray-500 mt-2">Vui lòng thử từ khóa khác hoặc gửi yêu cầu hỗ trợ bên dưới.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Support Form */}
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Gửi yêu cầu hỗ trợ</h2>
              <p className="text-sm text-gray-500 mb-6">Không tìm thấy câu trả lời? Hãy gửi yêu cầu cho chúng tôi. Đội ngũ hỗ trợ sẽ phản hồi sớm nhất.</p>
              
              <form onSubmit={handleFeedbackSubmit} className="bg-[#141417] border border-white/5 rounded-2xl p-6 md:p-8">
                
                <div className="mb-6">
                  {/* Category Select */}
                  <div>
                    <label className="block text-[13px] font-medium text-gray-400 mb-2">Phân loại vấn đề <span className="text-red-500">*</span></label>
                ...
                {/* Content Textarea */}
                <div className="mb-6">
                  <label className="block text-[13px] font-medium text-gray-400 mb-2">Mô tả chi tiết <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <textarea 
                    ...
                    />
                    <span className="absolute bottom-3 right-4 text-xs text-gray-600">{formData.content.length}/2000</span>
                  </div>
                </div>

                {/* Attachment */}
                <div className="mb-8">
                  <label className="block text-[13px] font-medium text-gray-400 mb-2">Đính kèm hình ảnh (tùy chọn)</label>
                  <input
                    type="file"
                    ref={fileInputRef}
                  ...
                  {formData.attachmentUrl ? (
                     <div className="relative w-full rounded-xl overflow-hidden border border-white/10 group">
                       <img src={formData.attachmentUrl} alt="Đính kèm" className="w-full max-h-[200px] object-contain bg-[#0a0a0c] p-2" />
                    ...
                    >
                      ...
                      <div className="text-sm">
                        ...
                        <button type="button" className="bg-white/10 text-gray-200 px-4 py-1.5 rounded-lg hover:bg-white/20 transition-colors text-xs font-medium cursor-pointer">
                          Chọn file
                        </button>
                        <p className="text-gray-600 text-[11px] mt-3">Hỗ trợ: JPG, PNG, GIF (tối đa 5MB mỗi file)</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4 -rotate-45 mb-1" />}
                    GỬI YÊU CẦU HỖ TRỢ
                  </button>
                </div>
              </form>
          ...
          <div className="w-full lg:w-[35%] flex flex-col gap-6">
            ...
            <div className="bg-[#141417] border border-white/5 rounded-2xl p-6">
              <h2 className="text-lg font-bold text-white mb-2">Thông tin hỗ trợ</h2>
              <p className="text-sm text-gray-400 mb-6 leading-relaxed">Chúng tôi luôn sẵn sàng hỗ trợ bạn 24/7. Hãy liên hệ qua các kênh dưới đây nếu cần hỗ trợ khẩn cấp.</p>
              ...
              <div className="space-y-6">
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="text-[14px] font-semibold text-gray-200">Email hỗ trợ</h3>
                    <p className="text-[13px] text-blue-400 font-medium">{siteConfig.contactEmail}</p>
                    <p className="text-[12px] text-gray-500 mt-0.5">Phản hồi trong 24h</p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="text-[14px] font-semibold text-gray-200">Trò chuyện trực tuyến</h3>
                    <p className="text-[13px] text-gray-400">Chat với nhân viên hỗ trợ</p>
                    <p className="text-[12px] text-gray-500 mt-0.5">Thời gian: 8:00 - 22:00</p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="text-[14px] font-semibold text-gray-200">Hotline</h3>
                    <p className="text-[13px] text-gray-400">{siteConfig.supportPhone}</p>
                    <p className="text-[12px] text-gray-500 mt-0.5">Thời gian: 8:00 - 22:00</p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-br from-[#2a1354] to-[#1a0f35] border border-purple-500/20 rounded-2xl p-6 relative overflow-hidden">
              ...
              <h2 className="text-lg font-bold text-white mb-2 relative z-10">Cộng đồng VideoSharing</h2>
              <p className="text-[13px] text-purple-200/80 mb-6 leading-relaxed relative z-10">Tham gia cộng đồng để nhận hỗ trợ nhanh hơn từ người dùng khác.</p>
            ...
            <div className="bg-[#141417] border border-white/5 rounded-2xl p-6">
              ...
              <ul className="space-y-4">
                <li className="flex gap-3">
                  ...
                  <p className="text-[13px] text-gray-400 leading-relaxed">Mô tả chi tiết vấn đề để được hỗ trợ nhanh hơn</p>
                </li>
                <li className="flex gap-3">
                  <CheckCircle2 className="w-4 h-4 text-orange-500 shrink-0 mt-0.5" />
                  <p className="text-[13px] text-gray-400 leading-relaxed">Đính kèm hình ảnh giúp chúng tôi hiểu rõ hơn</p>
                </li>
                <li className="flex gap-3">
                  <CheckCircle2 className="w-4 h-4 text-orange-500 shrink-0 mt-0.5" />
                  <p className="text-[13px] text-gray-400 leading-relaxed">Kiểm tra câu hỏi thường gặp trước khi gửi</p>
                </li>
              </ul>
            </div>
