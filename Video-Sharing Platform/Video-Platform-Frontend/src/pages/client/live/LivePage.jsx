      ...
      <div className="max-w-[1600px] mx-auto p-2 md:p-2 lg:p-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">
                Khám phá các buổi livestream hấp dẫn đang diễn ra và sắp diễn ra
              </p>
            </div>
          </div>
          <Link
            to="/studio/live"
            className="flex items-center gap-2 bg-[#7B1FA2] hover:bg-[#6A1B9A] text-white px-5 py-2.5 rounded-lg font-semibold text-sm transition-colors w-fit shrink-0"
          >
        ...
        <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide mb-8 pb-2">
          ...
          {categories.map((cat, idx) => {
            const IconComponent =
              LucideIcons[cat.icon] || LucideIcons.LayoutGrid;
          ...
          <div className="flex flex-col gap-4">
            ...
            {featuredMain && (
              <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1.2fr] gap-4 h-auto lg:h-[400px]">
                {/* Main Hero Card */}
                <Link
                  to={`/live/${featuredMain.id}`}
                  className="relative rounded-2xl overflow-hidden group border border-white/5 h-[300px] lg:h-full block"
                >
                  <img
                    src={
                      featuredMain.thumbnailUrl ||
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    ...
                    <span className="bg-black/60 backdrop-blur text-white text-xs font-bold px-2 py-1 rounded flex items-center gap-1">
                      ...
                      {formatViews(featuredMain.currentViewers || 0)}
                    </span>
                  </div>
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    ...
                    <h2 className="text-[26px] font-bold text-white mb-3 line-clamp-2 leading-tight">
                      {featuredMain.title}
                    </h2>
                    <div className="flex items-center gap-2 mb-4">
                      <img
                        src={
                          featuredMain.channel?.avatarUrl ||
                          `https://ui-avatars.com/api/?name=${featuredMain.channel?.channelName}`
                        }
                      ...
                      <span className="text-gray-300 font-medium text-sm flex items-center gap-1">
                        ...
                        {featuredMain.channel?.isVerified === true && (
                          <LucideIcons.CheckCircle className="w-3 h-3 text-white fill-green-500 shrink-0" />
                        )}
                      </span>
                    </div>
                    <p className="text-gray-400 w-90 text-sm mb-5 line-clamp-2 max-w-lg hidden sm:block">
                      {featuredMain.description ||
                        "Cùng đồng đội chinh phục top 1 và những pha xử lý đỉnh cao! Tham gia ngay để trò chuyện cùng mình nhé."}
                    </p>
                ...
                <div className="flex flex-col gap-4 h-full">
                  {featuredSide.map((stream, idx) => (
                    <Link
                      to={`/live/${stream.id}`}
                      key={idx}
                      className="flex-1 bg-[#18181C] rounded-2xl border border-white/5 overflow-hidden flex group hover:bg-white/5 transition-colors"
                    >
                      ...
                      <div className="p-4 flex flex-col justify-center flex-1 min-w-0">
                        <h3 className="text-sm font-bold text-white mb-2 line-clamp-2 leading-snug group-hover:text-purple-400 transition-colors">
                          {stream.title}
                        </h3>
                        <div className="flex items-center gap-1.5 mb-2">
                          ...
                          {stream.channel?.isVerified === true && (
                            <LucideIcons.CheckCircle className="w-3 h-3 text-white fill-green-500 shrink-0" />
                          )}
                        </div>
                        <div className="flex items-center justify-between mt-auto">
                          <span className="text-gray-500 text-[11px]">
                            {formatViews(stream.currentViewers)} người xem
                          </span>
            ...
            <div>
                  ...
                  >
                    <div className="relative aspect-video rounded-xl overflow-hidden bg-[#18181C] border border-white/5">
                      ...
                      <div className="absolute top-2 right-2 bg-black/60 backdrop-blur text-white text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1">
                        ...
                        {formatViews(stream.currentViewers)}
                      </div>
                    </div>
                    <div className="flex gap-3 px-1">
                      <img
                        src={
                          stream.channel?.avatarUrl ||
                          `https://ui-avatars.com/api/?name=${stream.channel?.channelName}`
                        }
                        className="w-9 h-9 rounded-full object-cover shrink-0 border border-white/10"
                        alt="avatar"
                      />
                      <div className="flex flex-col overflow-hidden">
                        <h3 className="text-sm font-bold text-white leading-tight mb-1 line-clamp-2 group-hover:text-purple-400 transition-colors">
                          {stream.title}
                        </h3>
              ...
              <div className="mt-8 flex justify-center">
                <button className="flex items-center gap-2 border border-white/10 hover:bg-white/5 text-white px-6 py-2.5 rounded-full text-sm font-medium transition-colors">
                  Xem tất cả livestream đang diễn ra{" "}
                  ...
                  return (
                    <div key={idx} className="flex gap-3 group cursor-pointer">
                      ...
                      <div className="flex flex-col flex-1 min-w-0 py-0.5">
                        <h3 className="text-[13px] font-bold text-white leading-tight mb-1.5 line-clamp-2 group-hover:text-purple-400 transition-colors">
                          {stream.title}
                        </h3>
            ...
            <div className="bg-[#141418] rounded-2xl border border-white/5 p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {POPULAR_TOPICS.map((topic, idx) => (
                  <button
                    key={idx}
                    className="bg-[#1F1F25] hover:bg-[#2A2A32] text-gray-300 hover:text-white border border-white/5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors shadow-sm"
                  >
                    {topic}
                  </button>
                ))}
